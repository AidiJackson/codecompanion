...
class AgentCapability(BaseModel):
    ...
    secondary_tasks: List[TaskType] = Field(default_factory=list, description="Secondary task types")
    
    # Supported artifact types
    produces_artifacts: List[ArtifactType] = Field(..., description="Artifact types this agent produces")
    consumes_artifacts: List[ArtifactType] = Field(default_factory=list, description="Artifact types this agent consumes")
    
    # Performance characteristics
    avg_processing_time_minutes: float = Field(..., description="Average processing time")
    quality_score: float = Field(..., ge=0.0, le=1.0, description="Historical quality score")
    ...
    preferred_complexity_level: str = Field(default="medium", description="Preferred task complexity")
...
class AgentInput(BaseModel):
    ...
    correlation_id: Optional[str] = Field(None, description="Correlation ID for related requests")
    ...
    context_handles: List[str] = Field(default_factory=list, description="Context handle IDs for retrieval")
    ...
    preferences: Dict[str, Any] = Field(default_factory=dict, description="Processing preferences")
    
    # Quality requirements
    quality_threshold: float = Field(default=0.8, ge=0.0, le=1.0, description="Minimum quality threshold")
    completeness_threshold: float = Field(default=0.9, ge=0.0, le=1.0, description="Minimum completeness threshold")
    
    # Execution constraints
    max_processing_time: Optional[int] = Field(None, description="Maximum processing time in seconds")
...
class AgentOutput(BaseModel):
    ...
    # Quality metrics
    confidence: float = Field(..., ge=0.0, le=1.0, description="Confidence in the output")
    ...
    intermediate_results: List[Dict[str, Any]] = Field(default_factory=list, description="Intermediate processing results")
    
    # Follow-up requests
    tests_requested: List[str] = Field(default_factory=list, description="Suggested tests for the artifact")
    review_points: List[str] = Field(default_factory=list, description="Points that need review")
    dependencies_identified: List[str] = Field(default_factory=list, description="Additional dependencies identified")
...
class BaseAgent(ABC):
    ...
    def can_handle_task(self, task_type: TaskType, artifact_type: ArtifactType) -> bool:
        ...
        task_compatible = (
            task_type in self.capabilities.primary_tasks or
            task_type in self.capabilities.secondary_tasks
        )
    ...
    def estimate_processing_time(self, agent_input: AgentInput) -> float:
        ...
        context_factor = min(2.0, len(agent_input.context) / 1000)  # Longer context = more time
        dependency_factor = 1.0 + (len(agent_input.dependency_artifacts) * 0.1)  # More deps = more time
    ...
    async def process_request(self, agent_input: AgentInput) -> AgentOutput:
        ...
        try:
            ...
            # Capability check
            if not self.can_handle_task(TaskType.REASONING_LONG, agent_input.requested_artifact):
                # Note: This is simplified - in real implementation, we'd infer task type
                logger.warning(f"Agent {self.agent_id} may not be optimal for {agent_input.requested_artifact}")
            
            # Process request
            logger.info(f"Processing request {agent_input.request_id} for task {agent_input.task_id}")
            ...
            self._record_processing_history(agent_input, processing_result.output, processing_time)
            
            return processing_result.output
            
        except Exception as e:
            logger.error(f"Unexpected error processing request {agent_input.request_id}: {e}")
            return self._create_error_output(agent_input, f"Unexpected error: {str(e)}", start_time)
    ...
    def _validate_common_input(self, agent_input: AgentInput) -> List[str]:
        ...
        if agent_input.requested_artifact not in self.capabilities.produces_artifacts:
            errors.append(f"Agent cannot produce {agent_input.requested_artifact.value}")
        
        return errors
    
    def _create_error_output(self, agent_input: AgentInput, error_message: str, 
                           start_time: datetime) -> AgentOutput:
    ...
    def _update_performance_metrics(self, output: AgentOutput, processing_time: float):
        ...
        self.performance_metrics["avg_processing_time"] = (
            (self.performance_metrics["avg_processing_time"] * (total_requests - 1) + processing_time) / 
            total_requests
        )
        
        success = output.status == "completed"
        self.performance_metrics["success_rate"] = (
            (self.performance_metrics["success_rate"] * (total_requests - 1) + (1.0 if success else 0.0)) /
            total_requests
        )
        
        if success:
            self.performance_metrics["avg_quality_score"] = (
                (self.performance_metrics["avg_quality_score"] * (total_requests - 1) + output.quality_score) /
                total_requests
            )
    
    def _record_processing_history(self, agent_input: AgentInput, output: AgentOutput, 
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_performance": {
                "avg_processing_time": sum(h["processing_time"] for h in recent_history) / max(len(recent_history), 1),
                "avg_quality_score": sum(h["quality_score"] for h in recent_history) / max(len(recent_history), 1),
                "success_rate": sum(1 for h in recent_history if h["status"] == "completed") / max(len(recent_history), 1)
            },
            "capabilities": {
                "primary_tasks": [task.value for task in self.capabilities.primary_tasks],
                "produces_artifacts": [art.value for art in self.capabilities.produces_artifacts],
