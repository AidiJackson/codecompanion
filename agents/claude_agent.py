...
class ClaudeAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "claude_agent"):
        capabilities = AgentCapability(
            ...
            secondary_tasks=[TaskType.CODE_REVIEW, TaskType.DEBUGGING],
            produces_artifacts=[
            ...
            consumes_artifacts=[ArtifactType.SPEC_DOC, ArtifactType.CODE_PATCH],
        ...
        logger.info(
            f"Claude agent initialized with {len(capabilities.produces_artifacts)} artifact types"
        )
    ...
    def _validate_input(self, agent_input: AgentInput) -> List[str]:
        ...
        if len(agent_input.context) > self.capabilities.max_context_length:
            errors.append(
                f"Context too long: {len(agent_input.context)} > {self.capabilities.max_context_length}"
            )
        ...
        if len(agent_input.context) < 100:
            errors.append(
                "Context too brief for quality analysis (minimum 100 characters)"
            )
    ...
    async def _process_request(self, agent_input: AgentInput) -> ProcessingResult:
        ...
        try:
            ...
            return ProcessingResult(
                ...
                resource_usage={
                    "estimated_tokens": len(agent_input.context)
                    + len(result.artifact.get("content", "")) * 1.3,
            ...
            logger.error(
                f"Claude processing error for request {agent_input.request_id}: {e}"
            )
            return ProcessingResult(
                success=False,
                error_message=str(e),
    ...
    async def _create_spec_doc(self, agent_input: AgentInput) -> AgentOutput:
        ...
        acceptance_criteria = self._generate_acceptance_criteria(
            agent_input.objective, requirements
        )
    ...
    async def _create_eval_report(self, agent_input: AgentInput) -> AgentOutput:
        ...
        eval_report_data = {
            ...
            "evaluated_artifact": agent_input.dependency_artifacts[0]
            if agent_input.dependency_artifacts
            else "unknown",
            "evaluation_criteria": [
    ...
    def _generate_acceptance_criteria(
        self, objective: str, requirements: List[Dict[str, Any]]
    ) -> List[str]:
    ...
    def _assess_risks(
        self, context: str, requirements: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    def _make_design_decisions(
        self, context: str, components: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    def _define_monitoring_points(self, context: str) -> List[Dict[str, str]]:
        ...
        return [
            ...
            },
            {
                "metric": "error_rate",
                "threshold": "< 1%",
                "action": "Page on-call if exceeded",
            },
            {
                "metric": "cpu_usage",
                "threshold": "< 80%",
                "action": "Scale if sustained",
            },
            {
                "metric": "memory_usage",
                "threshold": "< 85%",
