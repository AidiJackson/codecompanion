...
from schemas.artifacts import (
    ArtifactType, SpecDoc, DesignDoc, CodePatch, TestPlan, EvalReport, Runbook
)
from schemas.ledgers import TaskLedger, TaskStatus
from schemas.routing import ModelType, TaskType, TaskComplexity
from agents.base_agent import AgentInput, AgentOutput, ProcessingResult
...
class LiveAgentWorker(StreamConsumer):
    ...
    async def process_event(self, event: StreamEvent):
        ...
        elif event.event_type == EventType.TASK_ASSIGNED and event.agent_id == self.agent_id:
    ...
    async def _evaluate_task_assignment(self, event: StreamEvent):
        ...
        if self._can_handle_task_type(task_type):
            ...
            await self.event_bus.publish_event(EventStreamType.AGENT_STATUS, interest_event)
    ...
    async def _execute_assigned_task(self, event: StreamEvent):
        ...
        try:
            ...
            if result.success:
                ...
                await self.event_bus.publish_event(EventStreamType.ARTIFACTS, artifact_event)
                ...
                # Handle task failure
                await self._handle_task_failure(event.correlation_id, task_id, result.error_message)
...
class ClaudeWorker(LiveAgentWorker):
    ...
    def _can_handle_task_type(self, task_type: str) -> bool:
        ...
        claude_tasks = ['reasoning', 'architecture', 'documentation', 'debugging', 'analysis']
    ...
    def _build_task_prompt(self, task_data: Dict[str, Any]) -> str:
        ...
        objective = task_data.get('objective', task_data.get('goal', 'Complete the task'))
...
Format your response as a structured design document.
        ...
        elif 'specification' in task_type.lower() or 'requirements' in task_type.lower():
            return f"""
...
Please provide a comprehensive solution with clear explanations and practical implementation guidance.
    ...
    def _create_artifact_from_response(self, task_data: Dict[str, Any], content: str) -> Dict[str, Any]:
...
class GPT4Worker(LiveAgentWorker):
    ...
    def _can_handle_task_type(self, task_type: str) -> bool:
        ...
        gpt4_tasks = ['code', 'implementation', 'programming', 'development', 'ui', 'frontend']
    ...
    def _build_code_prompt(self, task_data: Dict[str, Any]) -> str:
        ...
        context = task_data.get('context', task_data.get('description', ''))
        task_type = task_data.get('primary_task_type', 'code')
...
Please provide the complete implementation with explanations.
    ...
    def _create_code_artifact(self, task_data: Dict[str, Any], content: str) -> Dict[str, Any]:
...
Format as a detailed test plan with executable test cases.
    ...
    def _create_test_artifact(self, task_data: Dict[str, Any], content: str) -> Dict[str, Any]:
...
class LiveAgentOrchestrator:
    ...
    async def assign_task_to_best_agent(self, task_data: Dict[str, Any], 
        ...
        for worker_name, worker in self.workers.items():
            if worker._can_handle_task_type(task_data.get('primary_task_type', '')):
                ...
                score = confidence - (0.1 * cost) - (0.05 * time_est / 60)  # Convert time to hours
        ...
        logger.info(f"Assigned task to {best_candidate['worker_name']} with score {best_candidate['score']:.3f}")
