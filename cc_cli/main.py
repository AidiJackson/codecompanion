...
def run(
    objective: str,
    outdir: Optional[str] = typer.Option(None, help="Folder to write artifacts"),
):
    ...
    (root / "manifest.json").write_text(
        json.dumps(
            {"run_id": run_id, "objective": objective, "artifacts": arts}, indent=2
        ),
        encoding="utf-8",
    )
...
def git_push(
    ...
    repo_url: Optional[str] = typer.Option(
        None, help="git remote URL (e.g., https://github.com/you/repo.git)"
    ),
    ...
    def run(cmd):
        ...
        if isinstance(cmd, str):
            ...
            cmd_list = cmd
        res = subprocess.run(
            cmd_list, shell=False, cwd=str(p), capture_output=True, text=True
        )
        if res.stdout:
