import os, sys, json, pathlib, urllib.request, urllib.error, time
API_URL = "https://code-companion-aidanjackson86.replit.app"
TOKEN   = os.getenv("CODECOMPANION_TOKEN") or "sullykevjackson050522"

def _slug(s): import re; s=re.sub(r"[^a-zA-Z0-9]+","-",s).strip("-").lower(); return s or f"task-{int(time.time())}"
def _save(goal, data):
    ...
    for a in data.get("artifacts") or []:
        name = {
            "SpecDoc":"SPEC.md","CodePatch":"PATCH.diff","TestPlan":"TESTPLAN.md",
            "DesignDoc":"DESIGN.md","EvalReport":"EVAL.md"
...
def main():
    if len(sys.argv)<2:
        print('Usage: ./codecompanion/ccmd "Your task"'); sys.exit(1)
    goal=" ".join(sys.argv[1:]).strip()
    try: data=_post("/run_real", {"objective":goal})
    except urllib.error.HTTPError as e:
        print(f"❌ HTTP {e.code}\n{e.read().decode('utf-8','ignore')[:800]}"); sys.exit(1)
