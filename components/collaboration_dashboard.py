    ...
    for task_id, task_info in active_tasks.items():
        ...
        with cols[col_idx]:
            ...
            if task_info["status"] == "processing":
                ...
                )  # Assume max 30s for visualization
...
def render_model_comparison():
    ...
    connected_models = {
        name: data for name, data in stats.items() if data["is_connected"]
    }
...
def render_consensus_sessions():
    ...
    for session in recent_sessions:
        with st.expander(
            f"Consensus Session {session['id'][-8:]} - Score: {session.get('consensus_score', 0):.1f}"
        ):
...
def render_agent_timeline():
    ...
    agent_specializations = {
        "PROJECT_MANAGER": {
            "model": "Claude",
            "color": "🟣",
            "tasks": ["Architecture planning", "Task coordination"],
        },
        "CODE_GENERATOR": {
            "model": "GPT-4",
            "color": "🔵",
            "tasks": ["Function implementation", "Algorithm design"],
        },
        "UI_DESIGNER": {
            "model": "GPT-4",
            "color": "🔵",
            "tasks": ["Component design", "Styling"],
        },
        "TEST_WRITER": {
            "model": "Gemini",
            "color": "🔴",
            "tasks": ["Test case generation", "Validation"],
        },
        "DEBUGGER": {
            "model": "Claude",
            "color": "🟣",
            "tasks": ["Bug analysis", "Code optimization"],
        },
    }
    ...
    if active_tasks:
        for task_id, task_info in active_tasks.items():
            ...
            if agent_type in agent_specializations:
                ...
                with col1:
                    st.write(
                        f"{agent_info['color']} {agent_type.replace('_', ' ').title()}"
                    )
    ...
    if st.button("🚀 Start Consensus Session", key="start_consensus"):
        with st.spinner("Consulting AI models..."):
            try:
                ...
                st.info(
                    "Consensus session initiated. Check the dashboard above for results."
                )
    ...
    with feed_container:
        ...
        if active_tasks:
            ...
            for task_id, task_info in list(active_tasks.items())[-5:]:  # Show last 5
                timestamp = datetime.fromtimestamp(task_info["start_time"]).strftime(
                    "%H:%M:%S"
                )
                ...
                st.write(
                    f"`{timestamp}` {status_icon} **{agent}** using {model} - {status}"
                )
        else:
...
def render_model_health_status():
    ...
    models_health = {
        ...
        "Claude": {
            "status": "Healthy"
            if health_status["anthropic"]["status"] == "healthy"
            else "Error",
        ...
        "Gemini": {
            "status": "Healthy"
            if health_status["google"]["status"] == "healthy"
            else "Error",
