...
from utils.session_manager import get_session_value, set_session_value, session_manager
...
class StabilityMonitor:
    ...
    def run_health_checks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(failed_checks) > 2:
                ...
                results['errors'].append(f"Multiple system failures: {', '.join(failed_checks)}")
            elif len(failed_checks) > 0:
                results['overall_health'] = 'warning'
                results['warnings'].extend([f"{check} needs attention" for check in failed_checks])
    ...
    def _check_memory_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_content_size = sum(len(str(msg.get('content', ''))) for msg in chat_history)
            total_content_size += sum(len(str(content)) for content in project_files.values())
    ...
    def render_health_dashboard(self):
        ...
        st.markdown(f"### {status_colors[overall_health]} System Status: {status_messages[overall_health]}")
        
        # Display individual checks
        if health_results['checks']:
            cols = st.columns(len(health_results['checks']))
            
            for i, (check_name, check_result) in enumerate(health_results['checks'].items()):
                with cols[i]:
