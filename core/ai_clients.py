...
class AIClientConfig(BaseModel):
    ...
    anthropic_api_key: Optional[str] = Field(
        default_factory=lambda: settings.ANTHROPIC_API_KEY
    )
    gemini_api_key: Optional[str] = Field(
        default_factory=lambda: settings.GEMINI_API_KEY
    )
...
class RealAIClients:
    ...
    async def call_claude_agent(
        self, agent_input: AgentInput, agent_type: AgentType
    ) -> AgentOutput:
        ...
        try:
            ...
            if agent_type == AgentType.PROJECT_MANAGER:
                prompt = AgentPrompts.PROJECT_MANAGER_PROMPT.format(
                    objective=agent_input.objective, context=agent_input.context
                )
            elif agent_type == AgentType.DEBUGGER:
                code_artifacts = (
                    "\n".join(agent_input.dependency_artifacts)
        ...
        try:
            ...
            response = self.openai_client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": f"You are a {agent_type.value} AI agent. Provide structured, actionable responses.",
                    },
    ...
    async def execute_agent(
        ...
        logger.info(
            f"Executing {agent_type.value} agent for task {agent_input.task_id}"
        )
