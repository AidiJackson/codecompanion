...
from typing import Dict, List, Optional, Any, Type, Union, Set
from uuid import uuid4
from pydantic import BaseModel, Field, ValidationError

from schemas.artifact_schemas import (
    ArtifactBase, ArtifactType, SpecDocSchema, DesignDocSchema, CodePatchSchema,
    TestPlanSchema, EvalReportSchema, RunbookSchema
)
...
class ArtifactLineage(BaseModel):
    ...
    parent_artifacts: List[str] = Field(default_factory=list, description="Parent artifact IDs")
    child_artifacts: List[str] = Field(default_factory=list, description="Child artifact IDs")
    creation_agent: str = Field(..., description="Agent that created this artifact")
    modification_history: List[Dict[str, Any]] = Field(default_factory=list, description="History of modifications")
    semantic_hash: str = Field(..., description="Hash of semantic content for change detection")
    version_chain: List[str] = Field(default_factory=list, description="Version evolution chain")
...
class ConfidenceMetrics(BaseModel):
    ...
    domain_confidence: float = Field(..., ge=0.0, le=1.0, description="Domain-specific confidence")
    completeness_confidence: float = Field(..., ge=0.0, le=1.0, description="Completeness confidence")
    accuracy_confidence: float = Field(..., ge=0.0, le=1.0, description="Accuracy confidence")
    consistency_confidence: float = Field(..., ge=0.0, le=1.0, description="Consistency confidence")
    
    # Factors affecting confidence
    agent_expertise_score: float = Field(default=0.8, ge=0.0, le=1.0, description="Agent's expertise in domain")
    data_quality_score: float = Field(default=0.8, ge=0.0, le=1.0, description="Quality of input data")
    complexity_penalty: float = Field(default=0.0, ge=0.0, le=0.5, description="Penalty for high complexity")
    ...
    def calculate_overall_confidence(self) -> float:
        ...
        adjusted_confidence = base_confidence * self.agent_expertise_score * self.data_quality_score
...
class TypedArtifactHandler:
    ...
    def create_artifact(self, agent_output: Dict[str, Any], artifact_type: str, 
        ...
        try:
            ...
            logger.error(f"Artifact validation failed for {artifact_type}: {error_details}")
    ...
    def _calculate_confidence_metrics(self, artifact: ArtifactBase, artifact_type: str, 
                                    agent_id: str, validation_result: ValidationResult) -> ConfidenceMetrics:
        ...
        data_quality = min(1.0, len(str(artifact.dict())) / 1000)  # Penalize very short artifacts
    ...
    def _calculate_domain_confidence(self, artifact: ArtifactBase, artifact_type: str) -> float:
        ...
        if artifact_type == "SpecDoc":
            ...
            if hasattr(spec, 'requirements') and spec.requirements:
                avg_req_quality = sum(1 for req in spec.requirements 
                                    if len(req.get('description', '')) > 30) / len(spec.requirements)
            ...
            if hasattr(patch, 'diff_unified') and patch.diff_unified:
                has_diff_markers = any(line.startswith(('+', '-', '@@')) 
    ...
    def _calculate_consistency_confidence(self, artifact: ArtifactBase) -> float:
        ...
        if title_words and content:
            consistency_ratio = len(title_words.intersection(content.split())) / len(title_words)
    ...
    def _generate_semantic_hash(self, artifact: ArtifactBase) -> str:
        ...
        semantic_dict = {k: v for k, v in semantic_content.items() 
    ...
    def _analyze_impact(self, artifact: ArtifactBase, artifact_type: str) -> Dict[str, Any]:
        ...
        impact = {
            ...
            "dependencies": artifact.dependencies if hasattr(artifact, 'dependencies') else []
        }
    ...
    def _check_semantic_conflict(self, artifact1: ArtifactBase, 
                               artifact2: ArtifactBase) -> Optional[Dict[str, Any]]:
