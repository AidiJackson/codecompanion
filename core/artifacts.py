...
class ValidationResult(BaseModel):
    ...
    validation_rules_applied: List[str] = Field(
        default_factory=list, description="Rules applied during validation"
    )
...
class ArtifactValidator:
    ...
    def validate_artifact(
        self, artifact_data: Dict[str, Any], validator_id: str = "system"
    ) -> ValidationResult:
        ...
        return ValidationResult(
            ...
            validation_rules_applied=[
                f"{artifact_type.value}_schema",
                f"{artifact_type.value}_quality",
            ],
        )
    ...
    def _validate_spec_doc_quality(self, spec: SpecDoc) -> Dict[str, Any]:
        ...
        if not spec.requirements:
            ...
            req_quality = sum(
                1 for req in spec.requirements if len(req.get("description", "")) > 20
        ...
        if not spec.acceptance_criteria:
            ...
            criteria_quality = sum(
                1 for criteria in spec.acceptance_criteria if len(criteria) > 10
        ...
        # Objective clarity
        objective_quality = min(
            1.0, len(spec.objective) / 100
        )  # Good objectives are descriptive
        quality_factors.append(objective_quality)

        # Requirements detail
        if spec.requirements:
            avg_req_length = sum(
                len(req.get("description", "")) for req in spec.requirements
            ) / len(spec.requirements)
    ...
    def _validate_design_doc_quality(self, design: DesignDoc) -> Dict[str, Any]:
        ...
        if not design.components:
            ...
            component_quality = sum(
                ...
                if all(key in comp for key in ["id", "name", "description"])
        ...
        if not design.design_decisions:
            ...
            decision_quality = sum(
                1 for decision in design.design_decisions if "rationale" in decision
            ) / len(design.design_decisions)
    ...
    def _validate_code_patch_quality(self, patch: CodePatch) -> Dict[str, Any]:
        ...
        if patch.breaking_changes:
            ...
            quality_factors.append(
                0.8
            )  # Neutral - no breaking changes or not documented
    ...
    def _validate_test_plan_quality(self, test_plan: TestPlan) -> Dict[str, Any]:
        ...
        if not test_plan.test_cases:
            ...
            complete_test_cases = sum(
                1 for tc in test_plan.test_cases if required_fields.issubset(tc.keys())
            )
    ...
    def _validate_runbook_quality(self, runbook: Runbook) -> Dict[str, Any]:
        ...
        if not runbook.procedures:
            ...
            complete_procedures = sum(
                1
                for proc in runbook.procedures
        ...
        if runbook.procedures:
            detailed_procedures = sum(
                1
                for proc in runbook.procedures
                if isinstance(proc.get("steps"), list)
                and len(proc.get("steps", [])) > 2
            )
...
class ArtifactHandler:
    ...
    def store_artifact(
        self, artifact_data: Dict[str, Any], validate: bool = True
    ) -> ValidationResult:
        ...
        if validate:
            ...
            if not validation_result.is_valid:
                logger.error(
                    f"Artifact validation failed: {validation_result.validation_errors}"
                )
        ...
        logger.info(
            f"Stored artifact {artifact_id} of type {artifact_data.get('artifact_type')}"
        )
        return validation_result
    ...
    def validate_dependencies(self, artifact_id: str) -> Dict[str, bool]:
        ...
        return {dep_id: dep_id in self.artifact_store for dep_id in dependencies}
    ...
    def get_artifact_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "orphaned_artifacts": len(
                [
                    artifact_id
                    for artifact_id in self.artifact_store.keys()
                    if artifact_id not in self.dependency_graph
                    and not self.get_dependents(artifact_id)
                ]
            ),
        }
