    ...
    def _update_context_weights(self, reward: float, context: Dict[str, Any]):
        ...
        for feature, value in context.items():
            if isinstance(value, (int, float)):
                ...
                updated_weight = (
                    1 - learning_rate
                ) * current_weight + learning_rate * correlation
...
class ThompsonSamplingBandit:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute(
                    ...
                    VALUES (?, ?, ?, ?)
                """,
                    (arm_id, reward, json.dumps(context), datetime.now().isoformat()),
                )
        except Exception as e:
            logger.error(f"Failed to record interaction: {e}")

    def register_arm(
        self, arm_id: str, prior_alpha: float = 1.0, prior_beta: float = 1.0
    ):
        """Register a new bandit arm (model)"""
        if arm_id not in self.arms:
            self.arms[arm_id] = BanditArm(
                arm_id=arm_id, alpha=prior_alpha, beta=prior_beta
            )
    ...
    def select_arm(
        ...
        logger.debug(
            f"Selected arm {selected_arm} with score {arm_scores[selected_arm]:.3f}"
        )

        return selected_arm

    def _calculate_context_score(
        self, arm: BanditArm, context: Dict[str, float]
    ) -> float:
