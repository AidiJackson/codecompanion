...
class MessageQueue:
    ...
    def add_message(self, message: Message) -> bool:
        ...
        if len(self.messages) >= self.max_size:
            ...
            self.messages = [
                m for m in self.messages if m.priority != MessagePriority.LOW
            ]
    ...
    def get_messages(
        self, recipient: str = None, message_type: MessageType = None
    ) -> List[Message]:
        ...
        if recipient:
            messages = [
                m for m in messages if m.recipient == recipient or m.recipient == "all"
            ]
...
class AgentCommunication:
    ...
    def get_conversation_history(
        self, agent1: str, agent2: str = None, limit: int = 50
    ) -> List[Dict[str, Any]]:
    ...
    def create_collaboration_session(
        self, session_name: str, participating_agents: List[str], goal: str
    ) -> Dict[str, Any]:
        ...
        logger.info(
            f"Created collaboration session {session_id} with {len(participating_agents)} agents"
        )
        return session_info
    ...
    def get_communication_stats(self) -> Dict[str, Any]:
        ...
        most_active = sorted(
            conversation_activity.items(), key=lambda x: x[1], reverse=True
        )[:5]
    ...
    def cleanup_old_messages(self, days_old: int = 7):
        ...
        for conv_key in list(self.active_conversations.keys()):
            ...
            recent_messages = [
                msg for msg in messages if msg.timestamp.timestamp() > cutoff_time
            ]
...
def send_agent_message(
    ...
    return communication_system.send_message(
        sender, recipient, MessageType.REQUEST, content, priority
    )


def send_agent_handoff(
    from_agent: str, to_agent: str, content: str, context: Dict[str, Any] = None
) -> str:
