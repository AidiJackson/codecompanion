...
class ConsensusValidator:
    ...
    async def validate_consensus(self, artifacts: List[Dict[str, Any]], 
                               domain: ValidationDomain,
                               consensus_method: ConsensusMethod = ConsensusMethod.HYBRID) -> ConsensusResult:
        """
        ...
        logger.info(f"Starting consensus validation for artifact {artifact_id} in domain {domain.value}")
        ...
        if len(model_validations) < self.min_models_required:
            raise ValueError(f"Insufficient model validations: {len(model_validations)} < {self.min_models_required}")
        ...
        logger.info(f"Consensus validation completed for {artifact_id}: "
                   f"score={consensus_result.consensus_score:.3f}, "
                   f"passed={consensus_result.validation_passed}")
        
        return consensus_result
    
    async def _get_model_validations(self, artifact: Dict[str, Any], 
                                   domain: ValidationDomain) -> List[ModelValidation]:
        ...
        for model_name in available_models:
            validation = await self._simulate_model_validation(artifact, model_name, domain)
            if validation:
                model_validations.append(validation)
        
        return model_validations
    
    async def _simulate_model_validation(self, artifact: Dict[str, Any], 
                                       model_name: str, domain: ValidationDomain) -> Optional[ModelValidation]:
        """
        ...
        try:
            ...
            confidence_score = min(1.0, max(0.0, 
                base_score + model_adjustments.get(model_name, 0) + 
                domain_adjustments.get(domain, 0) + 
                np.random.normal(0, 0.05)  # Add some variance
            ))
            
            quality_score = min(1.0, max(0.0, 
                confidence_score + np.random.normal(0, 0.03)
            ))
            
            # Generate validation text
            validation_text = self._generate_validation_text(artifact, model_name, domain, quality_score)
            
            # Generate issues and recommendations
            issues_found = self._generate_issues(artifact, domain, quality_score)
            recommendations = self._generate_recommendations(artifact, domain, quality_score)
    ...
    def _generate_validation_text(self, artifact: Dict[str, Any], model_name: str, 
                                domain: ValidationDomain, quality_score: float) -> str:
        """Generate model-specific validation text"""
        
        artifact_type = artifact.get("type", "artifact")
        
        if quality_score >= 0.9:
            return f"{model_name} validation: Excellent {artifact_type} quality in {domain.value} domain. " \
                   f"Well-structured with minimal issues identified."
        elif quality_score >= 0.8:
            return f"{model_name} validation: Good {artifact_type} quality in {domain.value} domain. " \
                   f"Some minor improvements recommended."
        elif quality_score >= 0.7:
            return f"{model_name} validation: Acceptable {artifact_type} quality in {domain.value} domain. " \
                   f"Several areas need attention."
        else:
            return f"{model_name} validation: Below standard {artifact_type} quality in {domain.value} domain. " \
                   f"Significant improvements required."
    
    def _generate_issues(self, artifact: Dict[str, Any], domain: ValidationDomain, 
        ...
        if quality_score < 0.9:
            ...
            available_issues = domain_issues.get(domain, ["Generic quality issues found"])
            issues = available_issues[:num_issues]
        
        return issues
    
    def _generate_recommendations(self, artifact: Dict[str, Any], domain: ValidationDomain,
                                quality_score: float) -> List[str]:
        ...
        if quality_score < 0.9:
            ...
            available_recs = domain_recommendations.get(domain, ["Consider general improvements"])
            recommendations = available_recs[:num_recommendations]
        
        return recommendations
    
    async def _apply_consensus_method(self, artifact_id: str, domain: ValidationDomain,
                                    model_validations: List[ModelValidation],
                                    consensus_method: ConsensusMethod) -> ConsensusResult:
        """Apply the specified consensus method"""
        
        if consensus_method == ConsensusMethod.WEIGHTED_VOTING:
            return await self._weighted_voting_consensus(artifact_id, domain, model_validations)
        elif consensus_method == ConsensusMethod.MAJORITY_VOTE:
            return await self._majority_vote_consensus(artifact_id, domain, model_validations)
        elif consensus_method == ConsensusMethod.SEMANTIC_SIMILARITY:
            return await self._semantic_similarity_consensus(artifact_id, domain, model_validations)
        elif consensus_method == ConsensusMethod.STATISTICAL_SIGNIFICANCE:
            return await self._statistical_significance_consensus(artifact_id, domain, model_validations)
    ...
    async def _weighted_voting_consensus(self, artifact_id: str, domain: ValidationDomain,
                                       model_validations: List[ModelValidation]) -> ConsensusResult:
        ...
        return ConsensusResult(
            ...
            statistical_metrics={
                ...
                "score_variance": float(np.var([v.quality_score for v in model_validations])),
                "confidence_variance": float(np.var([v.confidence_score for v in model_validations]))
            }
        )
    
    async def _majority_vote_consensus(self, artifact_id: str, domain: ValidationDomain,
                                     model_validations: List[ModelValidation]) -> ConsensusResult:
        ...
        avg_confidence = sum(v.confidence_score for v in model_validations) / total_votes
    ...
    async def _semantic_similarity_consensus(self, artifact_id: str, domain: ValidationDomain,
                                           model_validations: List[ModelValidation]) -> ConsensusResult:
        ...
        if len(validation_texts) < 2:
            ...
            avg_quality = sum(v.quality_score for v in model_validations) / len(model_validations)
            return ConsensusResult(
                artifact_id=artifact_id,
                domain=domain,
                consensus_score=avg_quality,
                final_quality_score=avg_quality,
                validation_passed=avg_quality >= self.consensus_thresholds.get(domain, 0.85),
        ...
        try:
            ...
            final_quality_score = float(sum(weighted_scores) / sum(similarities[i].sum() - 1 for i in range(n)) * (n - 1))
            consensus_score = float(total_similarity)
            
        except Exception as e:
            logger.warning(f"Error in semantic similarity analysis: {e}, falling back to average")
            # Fall back to simple average
            final_quality_score = sum(v.quality_score for v in model_validations) / len(model_validations)
            consensus_score = final_quality_score
            total_similarity = 0.0
        
        validation_passed = bool(final_quality_score >= self.consensus_thresholds.get(domain, 0.85))
    ...
    async def _statistical_significance_consensus(self, artifact_id: str, domain: ValidationDomain,
                                                model_validations: List[ModelValidation]) -> ConsensusResult:
        ...
        consensus_score = max(0.0, min(1.0, (mean_quality - threshold) / (1 - threshold)))
    ...
    async def _hybrid_consensus(self, artifact_id: str, domain: ValidationDomain,
                              model_validations: List[ModelValidation]) -> ConsensusResult:
        """Hybrid consensus combining multiple methods"""
        
        # Get results from different methods
        weighted_result = await self._weighted_voting_consensus(artifact_id, domain, model_validations)
        statistical_result = await self._statistical_significance_consensus(artifact_id, domain, model_validations)
        
        # Combine results
        combined_quality_score = (weighted_result.final_quality_score * 0.6 + 
                                statistical_result.final_quality_score * 0.4)
        
        combined_consensus_score = (weighted_result.consensus_score * 0.6 +
                                  statistical_result.consensus_score * 0.4)
        
        # Validation passes if both methods agree or combined score is high
        validation_passed = ((weighted_result.validation_passed and statistical_result.validation_passed) or
                           combined_quality_score >= self.consensus_thresholds.get(domain, 0.85))
    ...
    def get_domain_expertise_weights(self, domain: ValidationDomain) -> Dict[str, float]:
        """Get expertise weights for a specific domain"""
        return self.domain_weights.get(domain, {}).copy()
    
    def update_domain_weights(self, domain: ValidationDomain, new_weights: Dict[str, float]) -> None:
        ...
        if total_weight > 0:
            ...
            logger.info(f"Updated domain weights for {domain.value}: {normalized_weights}")
    
    def get_validation_history(self, domain: Optional[ValidationDomain] = None,
                             limit: int = 100) -> List[ConsensusResult]:
    ...
    def get_consensus_statistics(self) -> Dict[str, Any]:
        ...
        passed_validations = sum(1 for r in self.validation_history if r.validation_passed)
        avg_consensus_score = sum(r.consensus_score for r in self.validation_history) / total_validations
        avg_quality_score = sum(r.final_quality_score for r in self.validation_history) / total_validations
        avg_processing_time = sum(r.processing_time for r in self.validation_history) / total_validations
