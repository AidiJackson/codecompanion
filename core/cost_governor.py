...
class CostGovernor:
    ...
    def check_budget(self, project_complexity: str, current_usage: Dict[str, Any],
        ...
        try:
            ...
            usage.cost_incurred = current_usage.get('cost_incurred', usage.cost_incurred)
            usage.requests_made = current_usage.get('requests_made', usage.requests_made)
            usage.agents_active = current_usage.get('agents_active', usage.agents_active)
            
            # Check each budget constraint
            violations = []
            
            if usage.tokens_used > budget.max_tokens:
                violations.append(f"Token limit exceeded: {usage.tokens_used}/{budget.max_tokens}")
            
            if usage.cost_incurred > budget.max_cost_usd:
                violations.append(f"Cost limit exceeded: ${usage.cost_incurred:.2f}/${budget.max_cost_usd}")
            
            if usage.agents_active > budget.max_agents:
                violations.append(f"Agent limit exceeded: {usage.agents_active}/{budget.max_agents}")
            
            if usage.requests_made > budget.max_requests:
                violations.append(f"Request limit exceeded: {usage.requests_made}/{budget.max_requests}")
            ...
            if violations:
                for violation in violations:
                    ...
                    self._record_budget_violation(
                        project_id, violation, budget, usage
                    )
    ...
    def can_afford_model(self, model_type: ModelType, context: Dict[str, Any],
                        project_id: str = "default") -> bool:
        """
        ...
        try:
            ...
            if projected_cost > budget.max_cost_usd:
                logger.info(f"Model {model_type.value} not affordable: "
            ...
            if projected_tokens > budget.max_tokens:
                logger.info(f"Model {model_type.value} would exceed token budget: "
    ...
    def record_usage(self, project_id: str, model_type: ModelType,
                    tokens_used: int, actual_cost: float,
                    complexity_level: str, metadata: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            logger.debug(f"Recorded usage: {project_id} used {tokens_used} tokens "
                        f"costing ${actual_cost:.3f} with {model_type.value}")
            
        except Exception as e:
            logger.error(f"Failed to record usage: {e}")
    
    def suggest_cost_optimization(self, project_id: str,
                                current_model: ModelType,
                                context: Dict[str, Any]) -> Optional[ModelType]:
        """
        ...
        try:
            ...
            logger.info(f"Cost optimization suggestion: {current_model.value} -> "
        ...
        try:
            ...
            return {
                ...
                'utilization': {
                    ...
                    'requests_percent': (usage.requests_made / budget.max_requests) * 100
                },
    ...
    def _record_budget_violation(self, project_id: str, violation: str,
                                budget: BudgetLimits, usage: UsageMetrics):
    ...
    def _record_cost_optimization(self, project_id: str, original_model: ModelType,
                                 optimized_model: ModelType, estimated_savings: float):
    ...
    def _maybe_send_cost_alert(self, project_id: str, violations: List[str]):
        ...
        recent_alerts = [
            alert_time for alert_time in self.cost_alerts_sent[project_id]
    ...
    def _calculate_time_remaining(self, usage: UsageMetrics, 
                                 budget: BudgetLimits) -> Dict[str, float]:
    ...
    def _calculate_projected_overage(self, usage: UsageMetrics,
                                   budget: BudgetLimits) -> Dict[str, float]:
    ...
    def set_custom_budget(self, complexity: ProjectComplexity, 
