...
class HandoffRule(BaseModel):
    ...
    required_artifacts: List[ArtifactType] = Field(
        ..., description="Required artifact types"
    )
    optional_artifacts: List[ArtifactType] = Field(
        default_factory=list, description="Optional artifacts"
    )
    conditions: List[str] = Field(
        default_factory=list, description="Additional conditions for handoff"
    )
...
class HandoffResult(BaseModel):
    ...
    validated_artifacts: List[str] = Field(
        default_factory=list, description="Successfully validated artifacts"
    )
    rejected_artifacts: List[str] = Field(
        default_factory=list, description="Rejected artifacts"
    )
...
class AgentHandoff:
    ...
    def validate_handoff(
        ...
        request_id = (
            f"handoff_{start_time.strftime('%Y%m%d_%H%M%S')}_{from_agent}_{to_agent}"
        )
        ...
        if not handoff_rule:
            return HandoffResult(
                ...
                status=HandoffStatus.REJECTED,
                validation_errors=[
                    f"No handoff rule defined for {from_agent} -> {to_agent}"
                ],
                recommendations=[
                    f"Define handoff protocol for {from_agent} -> {to_agent}"
                ],
                processing_time=(datetime.now() - start_time).total_seconds(),
            )
                ...
                valid_handoffs.append(
                    {
                        ...
                        "required_artifacts": [
                            art.value for art in rule.required_artifacts
                        ],
                        "optional_artifacts": [
                            art.value for art in rule.optional_artifacts
                        ],
    ...
    def get_handoff_metrics(self) -> Dict[str, Any]:
        ...
        successful_handoffs = sum(
            1 for h in self.handoff_history if h.status == HandoffStatus.COMPLETED
        )
        failed_handoffs = sum(
            1 for h in self.handoff_history if h.status == HandoffStatus.FAILED
        )

        avg_processing_time = (
            sum(h.processing_time for h in self.handoff_history) / total_handoffs
        )
