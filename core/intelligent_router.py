...
class IntelligentTaskRouter:
        ...
        try:
            ...
            routing_decision = {
                ...
                "alternatives": [
                    ...
                    for alt in sorted(
                        agent_evaluations, key=lambda x: x["final_score"], reverse=True
                    )[1:4]
                ],
            ...
            logger.info(
                f"Routed task {correlation_id} to {best_agent['agent_id']} (score: {best_agent['final_score']:.3f})"
            )
    ...
    def _extract_task_type(self, task_data: Dict[str, Any]) -> TaskType:
        ...
        if any(keyword in content for keyword in ["architecture", "design", "system"]):
            ...
            keyword in content for keyword in ["document", "spec", "requirements"]
        ):
    ...
    async def _evaluate_agent_for_task(
        self,
        agent_id: str,
        task_type: TaskType,
        ...
        if not performance:
            ...
            performance = AgentPerformance(
                agent_id=agent_id, model_type=ModelType.CLAUDE_SONNET
            )
            self.agent_performance[agent_id] = performance

        # Calculate capability scores
        quality_score = self._calculate_quality_score(
            performance, task_type, complexity
        )
        ...
        return {
            ...
            "specialization_match": performance.specialization_scores.get(
                task_type, 0.5
            ),
        }
    ...
    def _calculate_quality_score(
        ...
        base_quality = (
            performance.average_quality if performance.average_quality > 0 else 0.75
        )

        # Adjust for specialization
        specialization_bonus = (
            performance.specialization_scores.get(task_type, 0.0) * 0.2
        )
        ...
        quality_score = (
            (base_quality + specialization_bonus)
            * reliability_factor
            * complexity_factor
        )
    ...
    def _calculate_cost_score(
        self, performance: AgentPerformance, complexity: TaskComplexity
    ) -> float:
    ...
    def _calculate_latency_score(
        ...
        base_time = (
            performance.average_time if performance.average_time > 0 else 180
        )  # 3 minutes default
    ...
    def _calculate_confidence(
        self, performance: AgentPerformance, task_type: TaskType
    ) -> float:
        ...
        specialization_confidence = performance.specialization_scores.get(
            task_type, 0.5
        )
    ...
    def _select_optimal_agent(
        ...
        sorted_evaluations = sorted(
            evaluations, key=lambda x: x["final_score"], reverse=True
        )
    ...
    async def _publish_routing_event(
        self, correlation_id: str, decision: Dict[str, Any]
    ):
        """Publish routing decision event"""
    ...
    async def update_agent_performance(
        ...
        if task_result.get("success", False):
            ...
            if task_type:
                ...
                performance.specialization_scores[task_type] = self._update_average(
                    current_spec,
                    quality,
                    5,  # Weight recent performance more
                )
        else:
        ...
        logger.debug(
            f"Updated performance for {agent_id}: {performance.success_rate:.2f} success rate"
        )
    ...
    def adjust_routing_weights(
        self, quality_weight: float, cost_weight: float, latency_weight: float
    ):
        ...
        logger.info(
            f"Updated routing weights: Q={self.quality_weight:.2f}, "
