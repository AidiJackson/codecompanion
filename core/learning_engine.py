...
class PostgreSQLStore:
    ...
    def update_model_performance(
        self, model_name: str, task_type: str, metrics: Dict[str, Any]
    ):
...
class MAMLLearner:
    ...
    def meta_train(
        self, task_episodes: List[List[Tuple[List[float], float]]]
    ) -> Dict[str, Any]:
        ...
        logger.info(
            f"MAML meta-training completed: CV score = {cv_scores.mean():.3f} ± {cv_scores.std():.3f}"
        )
...
class ContinuousLearner:
    ...
    def __init__(
        self,
        db_path: str = "learning_engine.db",
    ...
    def _prepare_meta_learning_data(self, history: List[RoutingDecision]):
        ...
        for decision in history:
            if decision.quality_score is not None:
                task_key = (
                    decision.task_features.task_type,
    ...
    def track_outcome(
        ...
        routing_decision.outcome_type = (
            OutcomeType.SUCCESS if quality_score >= 0.8 else OutcomeType.PARTIAL_SUCCESS
        )
        routing_decision.quality_score = quality_score
        routing_decision.processing_time = (
            datetime.now(timezone.utc) - routing_decision.timestamp
        ).total_seconds()
        ...
        if (
            len(self.recent_decisions) >= self.min_samples_for_learning
            and len(self.recent_decisions) % 20 == 0
        ):
        ...
        logger.debug(
            f"Tracked outcome for decision {routing_decision.decision_id}: "
    ...
    def _update_model_performance(self, decision: RoutingDecision):
        ...
        aggregated = {
            "avg_quality_score": np.mean(metrics["quality_scores"])
    ...
    def optimize_routing(self, task_features: TaskFeatures) -> Dict[str, float]:
        ...
        try:
            ...
            for model in self.routing_weights.keys():
                ...
                if (
                    model in self.model_performance
                    and task_type in self.model_performance[model]
                ):
                    historical_score = self.model_performance[model][task_type][
                        "avg_quality_score"
                    ]
            ...
            if total_score > 0:
                ...
                for model in self.routing_weights.keys():
                    ...
                    smoothed_weights[model] = (
                        0.7 * optimized_weight + 0.3 * current_weight
                    )
