...
class ProjectMemory:
    ...
    def init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute("CREATE INDEX IF NOT EXISTS idx_agent ON memory_entries(agent_name)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_project ON memory_entries(project_id)")
    ...
    def get_agent_history(self, 
        ...
        session_entries = [
            entry.to_dict() for entry in self.session_memory
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                rows = conn.execute("""
                    ...
                    LIMIT ?
                """, (cutoff_time.isoformat(), limit)).fetchall()
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in rows:
                    ...
                    # Avoid duplicates
                    if not any(r['entry_id'] == entry_dict['entry_id'] for r in results):
    ...
    def store_agent_knowledge(self, 
                            agent_name: str, 
                            knowledge_type: str,
    ...
    def get_memory_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Entries by agent
                agent_counts = dict(conn.execute("""
    ...
    def _update_context_from_interaction(self, request: str, response: str, agent_name: str):
        ...
        if "file" in request_lower or "code" in request_lower:
            self.update_context("last_code_interaction", {
                'agent': agent_name,
                'request': request[:200],
        ...
        if any(keyword in request_lower for keyword in ["project", "goal", "requirement", "build"]):
            self.update_context("project_goals", {
                'latest_goal': request[:300],
                'defined_by': agent_name
            }, agent_name)
        
        # Track technology decisions
        if any(tech in request_lower for tech in ["python", "javascript", "react", "vue", "fastapi"]):
            tech_mentioned = [tech for tech in ["python", "javascript", "react", "vue", "fastapi"] 
                            if tech in request_lower]
            self.update_context("technologies_mentioned", {
