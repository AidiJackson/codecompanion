...
class ModelOrchestrator:
    ...
    def _initialize_clients(self):
        ...
        if GOOGLE_GENAI_AVAILABLE and os.environ.get("GEMINI_API_KEY"):
            try:
                self.clients["gemini"] = genai.Client(api_key=os.environ.get("GEMINI_API_KEY"))
    ...
    def get_optimal_model(self, agent_type: AgentType, task_context: str = "") -> str:
        ...
        if preferred_model in self.clients and self.model_status[preferred_model].is_connected:
            return preferred_model
        
        # Fallback logic based on task context
        available_models = [name for name, status in self.model_status.items() 
        ...
        best_model = max(available_models, key=lambda x: self.model_status[x].reliability)
        return best_model
    ...
    async def generate_response(self, 
        ...
        try:
            ...
            if len(self.active_tasks) > 100:
                oldest_tasks = sorted(self.active_tasks.items(), 
    ...
    async def multi_model_consensus(self, 
        ...
        if models is None:
            models = [name for name, status in self.model_status.items() 
                     if status.is_connected and name in self.clients]
        ...
        messages = [
            {"role": "system", "content": "Provide a clear, concise answer to the following question. Focus on the key decision factors."},
        ...
        for model in models:
            if model in self.clients:
                task = self.generate_response(
                    model, messages, AgentType.PROJECT_MANAGER, 
        ...
        self.consensus_sessions[consensus_id].update({
            ...
            "status": "completed",
            "duration": time.time() - self.consensus_sessions[consensus_id]["start_time"]
        })
        ...
        return {
            ...
            "responses": responses,
            "consensus_score": consensus_score,
            "recommendation": valid_responses[0] if valid_responses else "No valid responses received",
    ...
    def get_recent_consensus_sessions(self, limit: int = 5) -> List[Dict]:
        ...
        sessions = sorted(self.consensus_sessions.items(), 
                         key=lambda x: x[1]["start_time"], reverse=True)
