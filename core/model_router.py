...
class CapabilityVector:
    ...
    def _get_context_adjustment(self, task_type: str, context: Dict[str, Any]) -> float:
        ...
        if context.get('time_sensitive', False) and task_type in ['code_gen', 'debugging']:
...
# Performance tracker will be initialized externally to avoid circular imports
    ...
    def extract_features(self, task: Dict[str, Any]) -> Dict[str, float]:
        ...
        features['complexity_keywords'] = self._count_complexity_keywords(description) / 10
        features['code_keywords'] = self._count_code_keywords(description) / 10
        features['reasoning_keywords'] = self._count_reasoning_keywords(description) / 10
        ...
        features['estimated_tokens'] = min(1.0, context.get('estimated_tokens', 1000) / 10000)
    ...
    def _count_complexity_keywords(self, text: str) -> int:
        ...
        keywords = [
            ...
            'comprehensive', 'detailed', 'thorough', 'extensive', 'multi-step'
        ]
    ...
    def _count_code_keywords(self, text: str) -> int:
        ...
        keywords = [
            ...
            'algorithm', 'implementation', 'programming', 'development'
        ]
        return sum(1 for keyword in keywords if keyword.lower() in text.lower())
    ...
    def _count_reasoning_keywords(self, text: str) -> int:
        ...
        keywords = [
            'analyze', 'design', 'plan', 'strategy', 'architecture', 'evaluate',
            'assess', 'consider', 'research', 'investigate', 'think', 'reason'
        ]
        return sum(1 for keyword in keywords if keyword.lower() in text.lower())
    
    def route_task(self, task: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        for model_type, capability_vector in self.capability_vectors.items():
            ...
            try:
                ...
                exploration_bonus = self.exploration_factor if selected_arm == model_type.value else 0.0
            except Exception:
            ...
            cost_component = capability_vector.base_capabilities.get('cost', 0.5) * self.cost_weight
            latency_component = capability_vector.base_capabilities.get('latency', 0.5) * self.latency_weight
            
            # Combine scores with exploration bonus
            final_score = quality_component + cost_component + latency_component + exploration_bonus
        ...
        logger.info(f"Routed task to {best_model.value} with score {model_scores[best_model]:.3f}")
        
        return routing_decision
    
    def _select_cost_efficient_model(self, features: Dict[str, float], 
    ...
    def _calculate_reward(self, outcome: TaskOutcome) -> float:
        ...
        speed_efficiency = min(1.0, max_reasonable_time / max(outcome.execution_time, 1.0))
    ...
    def _update_capability_vectors(self, outcome: TaskOutcome):
        ...
        logger.debug(f"Updated {outcome.model_used.value} weight for {task_type}: "
