...
from schemas.ledgers import TaskLedger, ProgressLedger, WorkItem, TaskStatus
from schemas.artifacts import ArtifactBase, ArtifactType
from schemas.routing import ModelType, TaskType, RoutingDecision
...
class WorkflowEvent(BaseModel):
    ...
    caused_by_event: Optional[str] = Field(None, description="ID of event that caused this one")
...
class OrchestratorState(BaseModel):
    ...
    # Task management
    tasks: Dict[str, TaskLedger] = Field(default_factory=dict, description="All tasks in workflow")
    task_dependencies: Dict[str, List[str]] = Field(default_factory=dict, description="Task dependency graph")
    ...
    # Agent assignments
    agent_assignments: Dict[str, List[str]] = Field(default_factory=dict, description="Agent to task mapping")
    agent_status: Dict[str, str] = Field(default_factory=dict, description="Current agent status")
    
    # Artifacts
    produced_artifacts: Dict[str, ArtifactBase] = Field(default_factory=dict, description="All produced artifacts")
    artifact_dependencies: Dict[str, List[str]] = Field(default_factory=dict, description="Artifact dependency graph")
    
    # Communication log
    communications: List[Dict[str, Any]] = Field(default_factory=list, description="Agent communications")
    
    # Progress tracking
    progress_ledger: Optional[ProgressLedger] = Field(None, description="Overall progress tracking")
    
    # Error tracking
    errors: List[Dict[str, Any]] = Field(default_factory=list, description="Workflow errors")
    warnings: List[Dict[str, Any]] = Field(default_factory=list, description="Workflow warnings")
...
class EventSourcedOrchestrator:
    ...
    def __init__(self, workflow_id: str):
        ...
        self.snapshots: Dict[int, OrchestratorState] = {}  # Event count -> state snapshot
    ...
    def _register_default_handlers(self):
        ...
        self.event_handlers[EventType.TASK_COMPLETED].append(self._handle_task_completed)
        self.event_handlers[EventType.TASK_FAILED].append(self._handle_task_failed)
        self.event_handlers[EventType.ARTIFACT_PRODUCED].append(self._handle_artifact_produced)
        self.event_handlers[EventType.AGENT_COMMUNICATION].append(self._handle_agent_communication)
        self.event_handlers[EventType.ERROR_OCCURRED].append(self._handle_error_occurred)
    ...
    def _emit_error_event(self, error_message: str, related_event: Optional[str] = None):
    ...
    def create_task(self, task: TaskLedger, correlation_id: Optional[str] = None) -> str:
    ...
    def assign_task(self, task_id: str, agent_id: str, routing_decision: RoutingDecision,
    ...
    def start_task(self, task_id: str, agent_id: str, correlation_id: Optional[str] = None) -> bool:
    ...
    def complete_task(self, task_id: str, agent_id: str, artifacts: List[ArtifactBase],
                     correlation_id: Optional[str] = None) -> bool:
    ...
    def fail_task(self, task_id: str, agent_id: str, error_message: str,
                 correlation_id: Optional[str] = None) -> bool:
        """Mark task as failed"""
    ...
    def log_communication(self, from_agent: str, to_agent: Optional[str], message: str,
                         communication_type: str = "handoff", correlation_id: Optional[str] = None):
