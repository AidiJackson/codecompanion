...
class EventSourcedOrchestrator:
    ...
    async def start_workflow(
        self, project_description: str, agents: List[AgentType]
    ) -> str:
    ...
    async def complete_task(
        self, task_id: str, output: Dict[str, Any], artifacts: List[ArtifactBase] = None
    ) -> str:
    ...
    async def publish_artifact(
        self, artifact: ArtifactBase, created_by: str, task_id: str = None
    ) -> str:
...
async def example_orchestration():
    ...
    orchestrator = EventSourcedOrchestrator(
    ...
    pm_task = await orchestrator.assign_task(
        AgentType.PROJECT_MANAGER, "Create project architecture and requirements"
    )
    ...
    await orchestrator.complete_task(
        pm_task, {"architecture": "microservices", "database": "postgresql"}
    )
