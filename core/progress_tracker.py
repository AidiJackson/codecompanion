...
from core.event_streaming import EventBus, StreamEvent, EventType, EventStreamType, StreamConsumer
...
class LiveProgressTracker(StreamConsumer):
    ...
    async def _handle_agent_started(self, event: StreamEvent):
        ...
        progress.estimated_completion = progress.start_time + timedelta(seconds=estimated_duration)
        
        self.agent_progress[agent_id] = progress
        
        # Update collaboration metrics
        await self._update_collaboration_metrics(event.correlation_id, "agent_started", agent_id)
    ...
    async def _handle_agent_completed(self, event: StreamEvent):
        ...
        if agent_id in self.agent_progress:
            ...
            if progress.start_time:
                ...
                logger.info(f"Agent {agent_id} completed in {execution_time:.1f} seconds")
            
            # Update collaboration metrics
            await self._update_collaboration_metrics(event.correlation_id, "agent_completed", agent_id)
    ...
    async def _handle_artifact_created(self, event: StreamEvent):
        ...
        # Update collaboration metrics
        await self._update_collaboration_metrics(event.correlation_id, "artifact_created")
        ...
        logger.info(f"Artifact {artifact_event.artifact_id} created by {artifact_event.created_by}")
    ...
    async def _update_collaboration_metrics(self, correlation_id: str, 
                                         event_type: str, agent_id: Optional[str] = None):
        """Update collaboration metrics"""
    ...
    def get_live_progress_summary(self) -> Dict[str, Any]:
        ...
        active_agents = [p for p in self.agent_progress.values() if p.status == "running"]
        completed_agents = [p for p in self.agent_progress.values() if p.status == "completed"]
        
        # Calculate overall progress
        if self.agent_progress:
            overall_progress = sum(p.progress_percentage for p in self.agent_progress.values()) / len(self.agent_progress)
        else:
            overall_progress = 0.0
        
        # Recent artifacts (last 10)
        recent_artifacts = sorted(self.artifact_events, key=lambda x: x.creation_time, reverse=True)[:10]
    ...
    def get_collaboration_metrics(self, correlation_id: str) -> Optional[Dict[str, Any]]:
    ...
    def get_artifact_timeline(self, correlation_id: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        agent_health = {
            ...
            "active_agents": len([p for p in self.agent_progress.values() if p.status == "running"]),
            "completed_agents": len([p for p in self.agent_progress.values() if p.status == "completed"]),
            "failed_agents": len([p for p in self.agent_progress.values() if p.status == "failed"])
        }
        
        success_rate = (
            agent_health["completed_agents"] / agent_health["total_agents"] 
            if agent_health["total_agents"] > 0 else 0
        )
        
        return {
            "uptime_seconds": uptime,
            "events_processed": self.total_events_processed,
            "processing_rate": self.total_events_processed / uptime if uptime > 0 else 0,
