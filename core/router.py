...
class RoutingContext(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.now, description="Routing timestamp"
    )
    ...
    # System context
    available_models: List[ModelType] = Field(
        ..., description="Currently available models"
    )
    system_load: Dict[ModelType, float] = Field(
        default_factory=dict, description="Current system load per model"
    )
    ...
    preferences: Dict[str, float] = Field(
        default_factory=dict, description="Soft preferences"
    )

    # Budget and limits
    remaining_budget: Optional[float] = Field(None, description="Remaining cost budget")
    max_latency_tolerance: Optional[float] = Field(
...
class ModelSelection(BaseModel):
    ...
    # Fallback options
    fallback_models: List[Tuple[ModelType, str]] = Field(
        default_factory=list, description="Fallback options with reasons"
    )

    # Execution guidance
    recommended_timeout: float = Field(
    ...
    monitoring_points: List[str] = Field(
        default_factory=list, description="What to monitor during execution"
    )
...
class DataDrivenRouter:
    ...
    def analyze_task_complexity(self, task: TaskLedger) -> TaskComplexity:
        ...
        context_requirement = min(
            1.0, description_length / 5000
        )  # Higher for longer descriptions
        ...
        estimated_tokens = int(
            description_length * 1.3
        )  # Rough estimate including response
    ...
    def apply_load_balancing(
        self, routing_decision: RoutingDecision, context: RoutingContext
    ) -> RoutingDecision:
        ...
        if current_load > 0.8:  # 80% load threshold
            logger.info(
                f"Model {selected_model} heavily loaded ({current_load:.2f}), considering alternatives"
            )
            ...
            for alt_model, alt_score in routing_decision.alternatives:
                ...
                if (
                    alt_load < current_load - 0.3
                    and alt_score > routing_decision.final_score * 0.85
                ):
                    ...
                    new_decision.final_score = alt_score * (
                        1.0 - alt_load * 0.2
                    )  # Adjust for load
    ...
    def route_task(
        ...
        routing_decision = self.model_router.route_task(
            task_type, complexity, constraints
        )
        ...
        if self.check_recent_failures(routing_decision.selected_model):
            logger.warning(
                f"Model {routing_decision.selected_model} has recent failures, selecting alternative"
            )
            ...
            for alt_model, alt_score in routing_decision.alternatives:
                if not self.check_recent_failures(alt_model):
                    ...
                    routing_decision.routing_parameters["failure_avoidance_applied"] = (
                        True
                    )
                    break
        ...
        confidence_factors = {
            ...
            "load_factor": 1.0
            - self.current_loads.get(routing_decision.selected_model, 0.0),
            "reliability_factor": 1.0
            - (
                len(self.recent_failures.get(routing_decision.selected_model, []))
                / 10.0
            ),
            "task_match": 1.0
            if task_type
            in [
                model.best_task_types
...
"""
        ...
        logger.info(
            f"Routed task {task.task_id} to {routing_decision.selected_model} "
    ...
    def record_failure(self, model: ModelType, error_message: str):
        ...
        self.recent_failures[model] = [
            failure_time
            for failure_time in self.recent_failures[model]
            if failure_time.timestamp() > cutoff_time
        ]
    ...
    def get_routing_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "model_usage_distribution": {
                model: count / total_selections for model, count in model_usage.items()
            },
            "current_loads": self.current_loads.copy(),
            "recent_failures": {
                model: len(failures) for model, failures in self.recent_failures.items()
            },
            "avg_confidence": sum(
                sum(selection.confidence_factors.values())
                / len(selection.confidence_factors)
                for selection in self.routing_history
            )
