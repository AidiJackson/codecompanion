...
def create_sample_task(
    task_description: str, task_type: TaskType, complexity_level: float = 0.5
) -> Dict[str, Any]:
...
def simulate_task_execution(model_type: ModelType, task: Dict[str, Any]) -> TaskOutcome:
    ...
    perf = model_performance.get(
        model_type, {"quality": 0.7, "speed": 0.8, "reliability": 0.8}
    )
...
async def demo_intelligent_routing():
    ...
    for i, task in enumerate(demo_tasks, 1):
        ...
        try:
            ...
            if alternatives:
                alt_text = ", ".join(
                    [f"{alt[0].value} ({alt[1]:.3f})" for alt in alternatives]
                )
    ...
    if "current_usage" in usage_summary:
        ...
        print("  Current Usage:")
        print(
            f"    Tokens: {usage['tokens_used']:,} / {budget['max_tokens']:,} "
            f"({utilization['tokens_percent']:.1f}%)"
        )
        print(
            f"    Cost: ${usage['cost_incurred']:.2f} / ${budget['max_cost_usd']:.2f} "
            f"({utilization['cost_percent']:.1f}%)"
        )
        print(
            f"    Requests: {usage['requests_made']} / {budget['max_requests']} "
    ...
    try:
        ...
        if bandit_stats:
            ...
            print("\n🔍 AI Insights:")
            print(
                f"  Best performing model: {best_model[0]} (score: {best_model[1]['estimated_mean']:.3f})"
            )
...
def demo_cost_governance():
    ...
    if suggested_model:
        ...
        print(
            f"Potential Savings: ${savings:.4f} ({savings / original_cost * 100:.1f}%)"
        )
