...
async def demo_live_agent_collaboration():
    ...
    try:
        ...
        correlation_id = await orchestrator.start_live_project(
        ...
        if "agent_metrics" in metrics and metrics["agent_metrics"]["worker_details"]:
            print("\n   Agent Performance:")
            for agent_name, agent_data in metrics["agent_metrics"][
                "worker_details"
            ].items():
        ...
        if "routing_stats" in metrics:
            ...
            print("\n   Routing Statistics:")
            print(
                f"     Total Routing Requests: {routing_stats['total_routing_requests']}"
            )
            print(
                f"     Average Routing Time: {routing_stats['average_routing_time_ms']:.1f}ms"
            )
...
async def demo_individual_agents():
    ...
    if os.environ.get("ANTHROPIC_API_KEY"):
        ...
        try:
            ...
            if result.success:
                print(
                    f"   ✅ Claude Success: Quality {result.output.quality_score:.2f}"
                )
    ...
    if os.environ.get("GEMINI_API_KEY"):
        ...
        try:
            ...
            if result.success:
                print(
                    f"   ✅ Gemini Success: Quality {result.output.quality_score:.2f}"
                )
                print(f"   📊 Processing time: {result.processing_time:.1f}s")
                print(f"   💰 Cost: ${result.resource_usage.get('cost', 0):.4f}")
                print(f"   📄 Artifact: {result.output.artifact['artifact_type']}")
