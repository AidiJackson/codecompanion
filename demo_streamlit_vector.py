...
def main():
    ...
    if 'vector_integration' not in st.session_state:
        st.session_state.vector_integration = VectorMemoryIntegration("memory/streamlit_demo.db")
    ...
    with st.sidebar:
        ...
        content_type = st.selectbox("Content Type", ["Agent Interaction", "Project Artifact"])
        
        if content_type == "Agent Interaction":
            agent_name = st.selectbox("Agent", ["ProjectManager", "CodeGenerator", "UIDesigner", "TestWriter", "Debugger"])
            interaction_type = st.selectbox("Type", ["response", "request", "planning", "analysis"])
            ...
            artifact_type = st.selectbox("Artifact Type", ["code", "documentation", "design", "test"])
    ...
    with col1:
        ...
        if search_interactions:
            agent_filter = st.selectbox("Filter by Agent", ["All Agents"] + ["ProjectManager", "CodeGenerator", "UIDesigner", "TestWriter", "Debugger"])
    
    with col2:
        search_artifacts = st.checkbox("Search Artifacts", value=True)
        artifact_filter = None
        if search_artifacts:
            artifact_filter = st.selectbox("Filter by Type", ["All Types"] + ["code", "documentation", "design", "test"])
    ...
    if st.button("Search") and query.strip():
        with st.spinner("Searching..."):
            ...
            if search_interactions:
                ...
                interactions = integration.find_similar_interactions(query, agent_name=agent_name, top_k=5)
                
                if interactions:
                    for i, result in enumerate(interactions, 1):
                        metadata = result['metadata']
                        with st.expander(f"{i}. {metadata.get('agent_name', 'Unknown')} - {metadata.get('interaction_type', 'N/A')} (Score: {result['similarity_score']:.3f})"):
                            # Get full content
                            full_doc = integration.expand_context_handle(result['document_id'])
                            if full_doc:
                                st.write(full_doc['text_content'][:500] + "..." if len(full_doc['text_content']) > 500 else full_doc['text_content'])
            ...
            if search_artifacts:
                ...
                artifact_type = None if artifact_filter == "All Types" else artifact_filter
                artifacts = integration.find_similar_artifacts(query, artifact_type=artifact_type, top_k=5)
                
                if artifacts:
                    for i, result in enumerate(artifacts, 1):
                        metadata = result['metadata']
                        with st.expander(f"{i}. {metadata.get('title', 'Untitled')} - {metadata.get('artifact_type', 'N/A')} (Score: {result['similarity_score']:.3f})"):
                            # Get full content
                            full_doc = integration.expand_context_handle(result['document_id'])
                            if full_doc:
                                st.code(full_doc['text_content'][:500] + "..." if len(full_doc['text_content']) > 500 else full_doc['text_content'])
                                st.caption(f"Document ID: {result['document_id']}")
                else:
    ...
    st.write("Get context handles for agent queries (returns only references, not full content)")
    
    context_query = st.text_input("Context Query", placeholder="What context do you need?")
    agent_requesting = st.selectbox("Agent Requesting Context", ["DemoAgent", "ProjectManager", "CodeGenerator", "UIDesigner", "TestWriter", "Debugger"])
    
    if st.button("Get Context Handles") and context_query.strip():
        with st.spinner("Getting context..."):
            handles = integration.get_context_for_agent(agent_requesting, context_query, max_contexts=5)
            
            if handles:
                st.success(f"Provided {len(handles)} context handles to {agent_requesting}")
                    ...
                    if st.button(f"Preview Handle {i}", key=f"preview_{i}"):
                        ...
                        if expanded:
                            with st.expander(f"Content Preview - {handle}"):
                                ...
                                st.write(f"**Key Phrases:** {', '.join(expanded['key_phrases'])}")
                                st.write(f"**Importance:** {expanded['importance_score']:.2f}")
                                st.write("**Full Content:**")
                                st.text(expanded['text_content'][:300] + "..." if len(expanded['text_content']) > 300 else expanded['text_content'])
            else:
