...
def demo_system_integration():
    ...
    for artifact in artifacts:
        ...
        print(
            f"  ✅ Stored {artifact['type']} artifact: {artifact['title']} -> {handle}"
        )
    ...
    for scenario in search_scenarios:
        ...
        if interactions:
            ...
            for result in interactions:
                ...
                print(
                    f"    • {metadata.get('agent_name', 'Unknown')} ({result['similarity_score']:.3f}) - {metadata.get('interaction_type', 'N/A')}"
                )
        ...
        if artifacts:
            ...
            for result in artifacts:
                metadata = result["metadata"]
                print(
                    f"    • {metadata.get('title', 'Untitled')} ({result['similarity_score']:.3f}) - {metadata.get('artifact_type', 'N/A')}"
                )
    ...
    for query in context_queries:
        print(f"\nQuery: '{query}'")
        context_handles = integration.get_context_for_agent(
            "DemoAgent", query, max_contexts=3
        )
        ...
        if context_handles:
            ...
            for i, handle in enumerate(context_handles, 1):
                ...
                if expanded:
                    content_preview = (
                        expanded["text_content"][:80] + "..."
                        if len(expanded["text_content"]) > 80
