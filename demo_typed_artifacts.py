...
def demo_artifact_creation():
    ...
    result = artifact_handler.create_artifact(
        agent_output=spec_doc_data, artifact_type="SpecDoc", agent_id="project_manager"
    )

    if "error" not in result:
        print("✅ SpecDoc created successfully!")
        print(f"   Artifact ID: {result['artifact']['artifact_id']}")
        print(
            f"   Confidence: {result['confidence_metrics']['overall_confidence']:.2f}"
        )
...
def demo_design_doc_creation(spec_doc_id: str):
    ...
    result = artifact_handler.create_artifact(
        agent_output=design_doc_data, artifact_type="DesignDoc", agent_id="ui_designer"
    )

    if "error" not in result:
        print("✅ DesignDoc created successfully!")
        print(f"   Artifact ID: {result['artifact']['artifact_id']}")
        print(
            f"   Confidence: {result['confidence_metrics']['overall_confidence']:.2f}"
        )
...
+}""",
    ...
    if "error" not in result:
        print("✅ CodePatch created successfully!")
        print(f"   Artifact ID: {result['artifact']['artifact_id']}")
        print(
            f"   Confidence: {result['confidence_metrics']['overall_confidence']:.2f}"
        )
...
def demo_handoff_validation():
    ...
    result = handoff_protocol.validate_handoff(
        from_agent="project_manager", to_agent="code_generator", artifacts=[spec_id]
    )
...
def demo_conflict_detection():
    ...
    result1 = artifact_handler.create_artifact(
        spec_data_1, "SpecDoc", "project_manager"
    )
    result2 = artifact_handler.create_artifact(
        spec_data_2, "SpecDoc", "project_manager"
    )
    ...
    artifact_ids = [
        result1["artifact"]["artifact_id"],
        result2["artifact"]["artifact_id"],
    ]
    ...
    if conflicts:
        ...
        print(
            f"Resolution Status: {'✅ Success' if resolution.success else '❌ Failed'}"
        )
...
def main():
    ...
    try:
        ...
        if spec_id:
            design_id = demo_design_doc_creation(spec_id)
            if design_id:
                demo_code_patch_creation(design_id)
