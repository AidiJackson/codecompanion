...
from core.handoff_protocol import AgentHandoff, AgentType
...
class TypedArtifactOrchestrator:
    ...
    def create_artifact_from_agent_output(self, agent_id: str, agent_output: str, 
                                        artifact_type: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            artifact_data = self._parse_agent_output(agent_output, artifact_type, context)
            ...
            if "error" not in result:
                ...
                # Log success
                logger.info(f"Created {artifact_type} artifact {artifact_id} from {agent_id}")
    ...
    def detect_and_resolve_conflicts(self, artifact_ids: List[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            'requires_manual_review': len([c for c in conflicts if c.severity in ['high', 'critical']])
        }
    ...
    def get_system_metrics(self) -> Dict[str, Any]:
        ...
        if artifacts:
            ...
            for artifact_data in artifacts.values():
                ...
                # Count by type
                artifact_metrics['by_type'][artifact_type] = artifact_metrics['by_type'].get(artifact_type, 0) + 1
                
                # Count by agent
                artifact_metrics['by_agent'][created_by] = artifact_metrics['by_agent'].get(created_by, 0) + 1
                
                # Collect confidence and quality scores
                confidences.append(artifact_data['confidence_metrics']['overall_confidence'])
        ...
        handoff_metrics = {
            ...
            'successful_handoffs': len([h for h in handoff_history if h['result']['status'] == 'validated']),
            'most_common_handoffs': {}
        }
        
        if handoff_history:
            handoff_metrics['success_rate'] = handoff_metrics['successful_handoffs'] / handoff_metrics['total_handoffs']
            
            # Count handoff patterns
            for handoff in handoff_history:
                pattern = f"{handoff['from_agent']} → {handoff['to_agent']}"
                handoff_metrics['most_common_handoffs'][pattern] = handoff_metrics['most_common_handoffs'].get(pattern, 0) + 1
        ...
        for conflict_record in conflict_history:
            ...
            conflict_metrics['by_type'][conflict_type] = conflict_metrics['by_type'].get(conflict_type, 0) + 1
            conflict_metrics['by_severity'][severity] = conflict_metrics['by_severity'].get(severity, 0) + 1
        
        return {
            'artifacts': artifact_metrics,
            'handoffs': handoff_metrics,
            'conflicts': conflict_metrics,
            'system_health': self._calculate_system_health(artifact_metrics, handoff_metrics, conflict_metrics)
        }
    
    def _parse_agent_output(self, agent_output: str, artifact_type: str, 
        ...
        artifact_data = {
            ...
            'description': agent_output[:500] + "..." if len(agent_output) > 500 else agent_output,
    ...
    def _calculate_system_health(self, artifact_metrics: Dict, handoff_metrics: Dict, 
                               conflict_metrics: Dict) -> Dict[str, Any]:
        ...
        if conflict_metrics['total_conflicts'] > artifact_metrics['total_artifacts'] * 0.3:
            health_score -= 0.2
    ...
    def _generate_health_recommendations(self, health_score: float, issues: List[str]) -> List[str]:
        ...
        if health_score < 0.5:
            recommendations.append("Consider reviewing agent configuration and training")
        
        if "Low average artifact quality" in issues:
            recommendations.append("Implement stricter validation rules and agent feedback")
        ...
        if "High conflict frequency" in issues:
            recommendations.append("Improve agent coordination and communication protocols")
        
        if any("critical conflicts" in issue for issue in issues):
            recommendations.append("Immediate manual review required for critical conflicts")
        
        if not recommendations:
            recommendations.append("System is operating well - maintain current practices")
