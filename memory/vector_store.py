...
class VectorStore:
    ...
    def __init__(
        self,
        db_path: str = "memory/vector_memory.db",
        ...
        if not self.use_openai and self.tfidf_enabled and TfidfVectorizer is not None:
            ...
            logger.warning(
                "⚠️  No vector similarity available - install scikit-learn for TF-IDF fallback"
            )
    ...
    def _initialize_database(self):
        ...
        with sqlite3.connect(str(self.db_path)) as conn:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_doc_hash ON vector_documents(text_hash)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_handle_type ON context_handles(context_type)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_handle_score ON context_handles(importance_score)"
            )
            ...
            if query_embedding is not None:
                ...
                for doc in documents:
                    if doc["embedding_type"] == "openai" and doc["embedding_data"]:
                        doc_embedding = np.frombuffer(
                            doc["embedding_data"], dtype=np.float32
                        )
                        ...
                        results.append(
                            (doc["document_id"], float(similarity), metadata)
                        )
                ...
                try:
                    ...
                    for i, doc in enumerate(documents):
                        metadata = json.loads(doc["metadata"] or "{}")
                        results.append(
                            (doc["document_id"], float(similarities[i]), metadata)
                        )
    ...
    def get_context_handles(
        self, context_type: Optional[str] = None, min_importance: float = 0.0
    ) -> List[Dict[str, Any]]:
        """
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        with sqlite3.connect(str(self.db_path)) as conn:
            ...
            handle_count = cursor.execute(
                "SELECT COUNT(*) FROM context_handles"
            ).fetchone()[0]
