...
class TrendAnalysis:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "time_span_days": (max(self.time_periods) - min(self.time_periods)).days
...
class PerformanceTracker:
    ...
    def __init__(
        self, db_path: str = "performance_tracking.db", max_history_days: int = 90
    ):
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_outcomes_model_task ON task_outcomes(model_used, task_type)"
            )
            conn.execute(
    ...
    def _load_recent_data(self):
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                logger.info(
                    f"Loaded {len(self.recent_outcomes)} recent outcomes from database"
                )
    ...
    def record_outcome(self, outcome: TaskOutcome):
        ...
        try:
            ...
            if key not in self.metrics:
                self.metrics[key] = PerformanceMetrics(
                    outcome.model_used, outcome.task_type
                )
            ...
            logger.debug(
                f"Recorded outcome for {outcome.model_used.value} on {outcome.task_type.value}"
            )
        ...
        for model_type in ModelType:
            ...
            for metric_name, values in metrics_to_analyze:
                ...
                trend = self._calculate_trend(
                    metric_name, [o.timestamp for o in model_outcomes], values
                )
    ...
    def _calculate_rolling_success_rate(
        self, outcomes: List[TaskOutcome], window_size: int = 10
    ) -> List[float]:
    ...
    def _calculate_trend(
        self, metric_name: str, timestamps: List[datetime], values: List[float]
    ...
    def _generate_summary_insights(self) -> Dict[str, Any]:
        ...
        best_cost = min(
            self.metrics.values(), key=lambda m: m.cost_per_success or float("inf")
        )
        ...
        return {
            ...
            "data_freshness": max(
                m.last_task for m in self.metrics.values() if m.last_task
            ).isoformat(),
        }
    ...
    def _analyze_quality_patterns(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_quality_std": statistics.stdev(all_quality_scores)
