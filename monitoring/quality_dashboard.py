...
class QualityMonitoringDashboard:
    ...
    def _render_quality_overview(self):
        ...
        with col3:
            best_model = max(
                quality_stats["by_model"].items(),
                key=lambda x: x[1]["avg_quality"],
                default=("N/A", {"avg_quality": 0}),
            )
            st.metric(
        ...
        with col4:
            quality_range = (
                quality_stats["overall"]["max_quality"]
                - quality_stats["overall"]["min_quality"]
            )
            st.metric("Quality Score Range", f"{quality_range:.3f}", delta=None)
        ...
        if quality_stats["by_model"]:
            ...
            avg_qualities = [
                quality_stats["by_model"][model]["avg_quality"] for model in models
            ]
            metric_counts = [
                quality_stats["by_model"][model]["metric_count"] for model in models
            ]
        ...
        if recent_metrics:
            ...
            # Time series plot
            fig = px.line(
                df_metrics.groupby(["timestamp", "model_name"])["quality_score"]
                .mean()
                .reset_index(),
                x="timestamp",
                y="quality_score",
    ...
    def _render_cascade_pipeline(self):
        ...
        with col3:
            quality_range = quality_stats.get(
                "quality_score_range", {"min": 0.0, "max": 0.0}
            )
                ...
                [
                    {
                        ...
                        "Human Review": "Yes"
                        if cascade["requires_human_review"]
                        else "No",
        ...
        with st.expander("Cascade Configuration"):
            ...
            for complexity in TaskComplexity:
                for stage in CascadeStage:
                    if stage in [
                        ...
                        threshold = self.quality_cascade.confidence_thresholds.get(
                            complexity, {}
                        ).get(stage, 0.0)
            ...
            if thresholds_data:
                ...
                pivot_df = threshold_df.pivot(
                    index="Stage", columns="Complexity", values="Threshold"
                )
    ...
    def _render_consensus_validation(self):
        ...
        if consensus_stats["total_validations"] > 0:
            ...
            if selected_domain:
                weights = self.consensus_validator.get_domain_expertise_weights(
                    selected_domain
                )
        ...
        with st.expander("Recent Validation History"):
            recent_validations = self.consensus_validator.get_validation_history(
                limit=10
            )
                    ...
                    validation_data.append(
                        {
                            ...
                            "Method": validation.consensus_method.value.replace(
                                "_", " "
                            ).title(),
                            "Models": len(validation.model_validations),
                            "Timestamp": validation.timestamp.strftime(
                                "%Y-%m-%d %H:%M:%S"
                            ),
                        }
                    )
    ...
    def _render_learning_analytics(self):
        ...
        with col2:
            st.metric(
                "Successful Adaptations", learning_stats["successful_adaptations"]
            )

        with col3:
            adaptation_rate = (
                learning_stats["successful_adaptations"]
                / max(learning_stats["total_decisions"], 1)
                * 100
            )
        ...
        with col4:
            st.metric(
                "Learning Mode",
                learning_stats["learning_mode"].replace("_", " ").title(),
            )
        ...
        if weights:
            ...
            fig = go.Figure(
                data=[go.Bar(x=models, y=weight_values, marker_color="lightcoral")]
            )

            fig.update_layout(
                title="Current Model Routing Weights", yaxis_title="Weight", height=400
            )
        ...
        with st.expander("Meta-Learning Status"):
            st.write(
                f"**Meta-learner Trained:** {'Yes' if learning_stats['meta_learner_trained'] else 'No'}"
            )
            if learning_stats["last_meta_training"]:
                st.write(
                    f"**Last Meta-Training:** {learning_stats['last_meta_training']}"
                )
            else:
        ...
        if trends:
            ...
            if selected_model == "All Models":
                ...
                if all_data:
                    ...
                    daily_trends = (
                        trends_df.groupby([trends_df["timestamp"].dt.date, "model"])[
                            "quality_score"
                        ]
                        .mean()
                        .reset_index()
                    )
                    daily_trends["timestamp"] = pd.to_datetime(
                        daily_trends["timestamp"]
                    )
                ...
                if quality_data:
                    ...
                    task_summary = (
                        model_df.groupby("task_type")["value"]
                        .agg(["count", "mean", "std"])
                        .reset_index()
                    )
    ...
    def _render_system_configuration(self):
        ...
        with st.expander("Quality Cascade Settings"):
            ...
            if complexity_level:
                thresholds = self.quality_cascade.confidence_thresholds.get(
                    complexity_level, {}
                )
        ...
        with st.expander("Learning Engine Settings"):
            ...
            st.write(
                f"**Meta-learner Status:** {'Trained' if learning_stats['meta_learner_trained'] else 'Not Trained'}"
            )
        ...
        with st.expander("Data Management"):
            ...
            days_to_keep = st.number_input(
                "Days of data to keep", min_value=1, max_value=365, value=90
            )
