...
class SpecDocSchema(ArtifactBase):
    ...
    title: str = Field(
        ..., min_length=5, max_length=200, description="Specification title"
    )
    ...
    functional_requirements: List[Dict[str, Any]] = Field(
        ...
        description="Functional requirements with structured format",
    )
...
class DesignDocSchema(ArtifactBase):
    ...
    # Design overview
    title: str = Field(
        ..., min_length=5, max_length=200, description="Design document title"
    )
    overview: str = Field(
        ..., min_length=50, description="High-level architecture overview"
    )
    design_goals: List[str] = Field(
        default_factory=list, description="Key design objectives"
    )
    ...
    def validate_design_decisions(cls, v):
        ...
        for decision in v:
            ...
            if not required_fields.issubset(decision.keys()):
                raise ValueError(
                    f"Design decisions must have fields: {required_fields}"
                )
        return v
...
class CodePatchSchema(ArtifactBase):
    ...
    title: str = Field(
        ..., min_length=5, max_length=200, description="Patch title/summary"
    )
    description: str = Field(
        ..., min_length=20, description="Detailed description of changes"
    )
...
class TestPlanSchema(ArtifactBase):
    ...
    test_strategy: str = Field(
        ..., min_length=20, description="Overall testing approach"
    )
    test_levels: List[str] = Field(
        default_factory=list,
        description="Levels of testing (unit, integration, system, etc.)",
    )
    test_types: List[str] = Field(
        default_factory=list,
        description="Types of testing (functional, performance, security, etc.)",
    )
...
class EvalReportSchema(ArtifactBase):
    ...
    title: str = Field(
        ..., min_length=5, max_length=200, description="Evaluation report title"
    )
    ...
    methodology: str = Field(
        ..., min_length=20, description="Evaluation methodology used"
    )
    ...
    evaluation_framework: str = Field(
        default="", description="Framework or standard used"
    )
...
class RunbookSchema(ArtifactBase):
    ...
    purpose: str = Field(
        ..., min_length=20, description="What this runbook accomplishes"
    )
    ...
    prerequisites: List[str] = Field(
        default_factory=list,
        description="Required setup, permissions, and dependencies",
    )
