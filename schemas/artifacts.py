...
class ArtifactBase(BaseModel):
    ...
    dependencies: List[str] = Field(
        default_factory=list, description="IDs of artifacts this depends on"
    )
...
class SpecDoc(ArtifactBase):
    ...
    constraints: List[str] = Field(
        default_factory=list, description="Technical/business constraints"
    )
    ...
    @field_validator("requirements", "non_functional_requirements", "risks")
    @classmethod
...
class DesignDoc(ArtifactBase):
    ...
    artifact_type: Literal[ArtifactType.DESIGN_DOC] = Field(
        default=ArtifactType.DESIGN_DOC
    )
    ...
    interfaces: List[Dict[str, Any]] = Field(
        default_factory=list, description="API/interface definitions"
    )
    data_models: List[Dict[str, Any]] = Field(
        default_factory=list, description="Data structure definitions"
    )
    design_decisions: List[Dict[str, Any]] = Field(
        ..., description="Key architectural decisions"
    )
    trade_offs: List[Dict[str, Any]] = Field(
        default_factory=list, description="Trade-off analysis"
    )
    diagrams: List[Dict[str, str]] = Field(
        default_factory=list, description="Architectural diagrams"
    )
    technology_stack: Dict[str, str] = Field(
        default_factory=dict, description="Tech stack choices"
    )

    @field_validator("components", "design_decisions")
    @classmethod
...
class CodePatch(ArtifactBase):
    ...
    artifact_type: Literal[ArtifactType.CODE_PATCH] = Field(
        default=ArtifactType.CODE_PATCH
    )
    ...
    def validate_files(cls, v):
        ...
        for file_change in v:
            if not isinstance(file_change, dict) or not required_fields.issubset(
                file_change.keys()
            ):
...
class TestPlan(ArtifactBase):
    ...
    artifact_type: Literal[ArtifactType.TEST_PLAN] = Field(
        default=ArtifactType.TEST_PLAN
    )
    ...
    test_data: List[Dict[str, Any]] = Field(
        default_factory=list, description="Test data sets"
    )
    automation_level: str = Field(
        default="manual", description="Level of test automation"
    )
    tools_required: List[str] = Field(
        default_factory=list, description="Testing tools needed"
    )
    environment_setup: str = Field(
        default="", description="Test environment requirements"
    )
    ...
    def validate_test_cases(cls, v):
        ...
        for test_case in v:
            if not isinstance(test_case, dict) or not required_fields.issubset(
                test_case.keys()
            ):
...
class EvalReport(ArtifactBase):
    ...
    artifact_type: Literal[ArtifactType.EVAL_REPORT] = Field(
        default=ArtifactType.EVAL_REPORT
    )
...
class Runbook(ArtifactBase):
    ...
    prerequisites: List[str] = Field(
        default_factory=list, description="Required setup/permissions"
    )
    procedures: List[Dict[str, Any]] = Field(..., description="Step-by-step procedures")
    troubleshooting: List[Dict[str, Any]] = Field(
        default_factory=list, description="Common issues & fixes"
    )
    monitoring: List[Dict[str, str]] = Field(
        default_factory=list, description="What to monitor"
    )
    escalation_paths: List[str] = Field(
        default_factory=list, description="Who to contact for help"
    )
    maintenance_schedule: str = Field(
        default="", description="Regular maintenance tasks"
    )
    ...
    def validate_procedures(cls, v):
        ...
        for procedure in v:
            if not isinstance(procedure, dict) or not required_fields.issubset(
                procedure.keys()
            ):
