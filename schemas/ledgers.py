...
class WorkItem(BaseModel):
    ...
    assigned_agent: Optional[str] = Field(
        None, description="Agent assigned to this item"
    )
    ...
    dependencies: List[str] = Field(
        default_factory=list, description="IDs of dependent work items"
    )
    blockers: List[str] = Field(
        default_factory=list, description="Current blocking issues"
    )
    artifacts_produced: List[str] = Field(
        default_factory=list, description="Artifact IDs produced"
    )
...
class TaskLedger(BaseModel):
    ...
    mitigation_strategies: List[str] = Field(
        default_factory=list, description="Risk mitigation plans"
    )

    # Execution tracking
    work_items: List[WorkItem] = Field(
        default_factory=list, description="Breakdown of work items"
    )
    assigned_agents: List[str] = Field(
        default_factory=list, description="Agents working on this task"
    )
    dependencies: List[str] = Field(
    ...
    expected_artifacts: List[ArtifactType] = Field(
        ..., description="Expected artifact types"
    )
    ...
    def validate_acceptance_tests(cls, v):
        ...
        for test in v:
            if not isinstance(test, dict) or not required_fields.issubset(test.keys()):
                raise ValueError(
                    f"Acceptance tests must have fields: {required_fields}"
                )
        return v
    ...
    def calculate_progress(self) -> float:
        ...
        completed_items = sum(
            1 for item in self.work_items if item.status == TaskStatus.COMPLETED
        )
...
class ContextHandle(BaseModel):
    ...
    context_type: str = Field(
        ..., description="Type of context (conversation, file, etc.)"
    )
    ...
    compression_ratio: Optional[float] = Field(
        None, description="Compression ratio if applicable"
    )
...
class ProgressLedger(BaseModel):
    ...
    # Work item tracking
    work_items: List[WorkItem] = Field(
        default_factory=list, description="All work items"
    )
    assigned_agents: Dict[str, List[str]] = Field(
        default_factory=dict, description="Agent to work item mapping"
    )

    # Blocking and dependency tracking
    blockers: List[Dict[str, Any]] = Field(
        default_factory=list, description="System-wide blockers"
    )
    dependency_graph: Dict[str, List[str]] = Field(
        default_factory=dict, description="Work item dependencies"
    )

    # Artifact management
    artifacts_registry: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="Artifact metadata"
    )
    artifact_dependencies: Dict[str, List[str]] = Field(
        default_factory=dict, description="Artifact dependency mapping"
    )
    ...
    # Timeline tracking
    created_at: datetime = Field(
        default_factory=datetime.now, description="Ledger creation time"
    )
    ...
    def validate_blockers(cls, v):
        ...
        for blocker in v:
            if not isinstance(blocker, dict) or not required_fields.issubset(
                blocker.keys()
            ):
    ...
    def update_metrics(self):
        ...
        self.blocked_items = sum(
            1 for item in self.work_items if item.status == TaskStatus.BLOCKED
        )
        self.in_progress_items = sum(
            1 for item in self.work_items if item.status == TaskStatus.IN_PROGRESS
        )
...
class MemoryIndex(BaseModel):
    ...
    # Context management
    context_handles: List[ContextHandle] = Field(
        default_factory=list, description="Available context handles"
    )
    active_handles: Set[str] = Field(
        default_factory=set, description="Currently active context handles"
    )

    # Budget and resource management
    retrieval_budget: Dict[str, int] = Field(
        default_factory=dict, description="API call budgets per service"
    )
    storage_budget_mb: float = Field(default=100.0, description="Storage budget in MB")
    current_storage_mb: float = Field(
        default=0.0, description="Current storage usage in MB"
    )

    # Compression and optimization
    compression_rules: Dict[str, Any] = Field(
        default_factory=dict, description="Context compression rules"
    )
    auto_compress_threshold_mb: float = Field(
        default=50.0, description="Auto-compression threshold"
    )
    max_handle_age_days: int = Field(
        default=30, description="Max age for context handles"
    )

    # Access patterns
    access_patterns: Dict[str, List[datetime]] = Field(
        default_factory=dict, description="Handle access history"
    )
    priority_handles: List[str] = Field(
        default_factory=list, description="High-priority context handles"
    )

    created_at: datetime = Field(
        default_factory=datetime.now, description="Index creation time"
    )
    ...
    def needs_cleanup(self) -> bool:
        ...
        return (
            self.current_storage_mb > self.auto_compress_threshold_mb
            or self.get_storage_utilization() > 80.0
        )
