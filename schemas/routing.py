...
class CapabilityVector(BaseModel):
    ...
    reasoning_long: float = Field(
        ..., ge=0.0, le=1.0, description="Complex reasoning and analysis"
    )
    code_ui: float = Field(
        ..., ge=0.0, le=1.0, description="UI/UX development capability"
    )
    code_backend: float = Field(
        ..., ge=0.0, le=1.0, description="Backend development capability"
    )
    test_gen: float = Field(
        ..., ge=0.0, le=1.0, description="Test generation capability"
    )
    code_review: float = Field(
        ..., ge=0.0, le=1.0, description="Code review and analysis"
    )
    documentation: float = Field(..., ge=0.0, le=1.0, description="Technical writing")
    debugging: float = Field(
        ..., ge=0.0, le=1.0, description="Debugging and troubleshooting"
    )
    architecture: float = Field(
        ..., ge=0.0, le=1.0, description="System architecture design"
    )

    # Performance characteristics
    latency_score: float = Field(
        ..., ge=0.0, le=1.0, description="Speed score (higher = faster)"
    )
    cost_score: float = Field(
        ..., ge=0.0, le=1.0, description="Cost efficiency (higher = cheaper)"
    )
...
class TaskComplexity(BaseModel):
    ...
    technical_complexity: float = Field(
        ..., ge=0.0, le=1.0, description="Technical difficulty"
    )
    novelty: float = Field(
        ..., ge=0.0, le=1.0, description="How new/unusual the task is"
    )
    safety_risk: float = Field(
        ..., ge=0.0, le=1.0, description="Safety/security risk level"
    )
    context_requirement: float = Field(
        ..., ge=0.0, le=1.0, description="Amount of context needed"
    )
    interdependence: float = Field(
        ..., ge=0.0, le=1.0, description="Dependencies on other tasks"
    )

    # Task characteristics
    estimated_tokens: int = Field(..., description="Estimated token count")
    requires_reasoning: bool = Field(
        default=False, description="Requires complex reasoning"
    )
    requires_creativity: bool = Field(
        default=False, description="Requires creative thinking"
    )
...
class ModelCapability(BaseModel):
    ...
    best_task_types: List[TaskType] = Field(
        ..., description="Tasks this model excels at"
    )
    avoid_task_types: List[TaskType] = Field(
        default_factory=list, description="Tasks to avoid"
    )

    # Routing preferences
    preferred_for_safety: bool = Field(
        default=False, description="Preferred for safety-critical tasks"
    )
    preferred_for_speed: bool = Field(
        default=False, description="Preferred for time-critical tasks"
    )
    preferred_for_cost: bool = Field(
        default=False, description="Preferred for cost optimization"
    )
...
class RoutingDecision(BaseModel):
    ...
    # Alternative options
    alternatives: List[Tuple[ModelType, float]] = Field(
        default_factory=list, description="Alternative models with scores"
    )
    ...
    routing_parameters: Dict[str, float] = Field(
        ..., description="Routing algorithm parameters"
    )
    ...
    suggested_timeout: Optional[float] = Field(
        None, description="Suggested timeout in minutes"
    )
    retry_strategy: str = Field(
        default="exponential_backoff", description="Retry strategy if failed"
    )
...
class ModelRouter(BaseModel):
    ...
    quality_weight: float = Field(
        default=0.7, description="Weight for quality in routing decision"
    )
    cost_weight: float = Field(default=0.2, description="Weight for cost optimization")
    latency_weight: float = Field(
        default=0.1, description="Weight for speed optimization"
    )

    # Constraints
    max_cost_per_task: Optional[float] = Field(
        None, description="Maximum cost per task"
    )
    max_latency_ms: Optional[float] = Field(
        None, description="Maximum acceptable latency"
    )
    require_high_availability: bool = Field(
        default=True, description="Require high availability models"
    )

    def calculate_routing_score(
        self, model: ModelCapability, task_type: TaskType, complexity: TaskComplexity
    ) -> float:
        """
    ...
    def route_task(
        ...
        for model in self.available_models:
            ...
            if constraints:
                if (
                    ...
                    and model.cost_per_1k_tokens > constraints["max_cost"]
                ):
                    continue
                if (
                    constraints.get("min_availability")
                    and model.availability < constraints["min_availability"]
                ):
                    continue
        ...
        quality_score = capability_score * (
            1.0 + complexity.get_complexity_score() * 0.5
        )
        ...
        return RoutingDecision(
            ...
            alternatives=[
                (m.model_type, s) for m, s in model_scores[1:3]
            ],  # Top 3 alternatives
            task_type=task_type,
