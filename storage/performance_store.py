...
class PerformanceStore:
    ...
    def _initialize_database(self):
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_metrics_model_task ON performance_metrics(model_name, task_type)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_metrics_type ON performance_metrics(metric_type)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_aggregated_period ON aggregated_metrics(period, start_time)')
    ...
    def store_metric(self, metric: PerformanceMetric) -> bool:
        ...
        try:
            ...
            logger.debug(f"Stored metric: {metric.metric_type.value} = {metric.value} for {metric.model_name}")
            return True
    ...
    def compute_aggregated_metrics(self, period: AggregationPeriod,
                                  start_time: Optional[datetime] = None,
        ...
        logger.info(f"Computed {len(aggregated_metrics)} aggregated metrics for {period.value} period")
        return aggregated_metrics
    
    def get_model_performance_trends(self, model_name: Optional[str] = None,
    ...
    def get_quality_statistics(self) -> Dict[str, Any]:
        ...
        return {
            "overall": {
                ...
                "avg_quality": overall_stats[1] if overall_stats and overall_stats[1] else 0.0,
                "min_quality": overall_stats[2] if overall_stats and overall_stats[2] else 0.0,
                "max_quality": overall_stats[3] if overall_stats and overall_stats[3] else 0.0
            },
            "by_model": model_stats,
            "recent_trends": recent_trends
        }
    
    def store_learning_outcome(self, routing_decision_id: str, expected_quality: float,
                             actual_quality: float, outcome_type: str,
    ...
    def cleanup_old_metrics(self, days_to_keep: int = 90) -> int:
        ...
        try:
            ...
            cursor.execute('DELETE FROM learning_outcomes WHERE timestamp < ?', (cutoff_time,))
            ...
            logger.info(f"Cleaned up {total_deleted} old records (metrics: {deleted_metrics}, outcomes: {deleted_outcomes})")
    ...
    def export_metrics(self, output_file: str, model_name: Optional[str] = None,
                      start_time: Optional[datetime] = None,
