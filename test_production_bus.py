...
async def test_orchestrator_integration():
    ...
    try:
        ...
        task_id = await orchestrator.assign_task(
            AgentType.PROJECT_MANAGER, "Test task description"
        )

        await orchestrator.complete_task(task_id, {"result": "test completion"})

        # Check status
        status = await orchestrator.get_workflow_status()
        logger.info(
            f"✅ Orchestrator status: {status['progress_percentage']}% complete"
        )
