...
def render_live_activity_feed(progress_tracker: Optional[LiveProgressTracker] = None):
    ...
    with col1:
        auto_refresh = st.checkbox(
            "🔄 Auto-refresh (5s)", value=True, key="activity_refresh"
        )
    ...
    with col3:
        max_events = st.selectbox(
            "Show events", [10, 25, 50, 100], index=1, key="max_events"
        )
    ...
    with activity_container:
        if progress_tracker:
            ...
            with col4:
                ...
                st.metric("🎯 Progress", f"{overall_progress:.1f}%", delta=None)
                        ...
                        with col2:
                            ...
                            if agent["estimated_completion"]:
                                ...
                                if time_remaining.total_seconds() > 0:
                                    st.write(
                                        f"**ETA:** {time_remaining.seconds // 60}m {time_remaining.seconds % 60}s"
                                    )
...
def render_execution_progress(execution: Dict[str, Any]):
    ...
    with col3:
        st.metric(
            "Completed", f"{execution['completed_agents']}/{execution['total_agents']}"
        )
    ...
    for agent_id, details in agent_details.items():
        ...
        st.write(
            f"{status_icon} **{details['agent_type']}** - {details['status']} ({details['progress']:.1f}%){dep_text}"
        )


def render_agent_progress_card(
    agent: Dict[str, Any], progress_tracker: LiveProgressTracker
):
...
def render_collaboration_metrics(
    progress_tracker: Optional[LiveProgressTracker] = None,
):
    ...
    if progress_tracker:
        ...
        if all_metrics:
            ...
            total_handoffs = sum(
                m.get("handoffs_completed", 0) for m in all_metrics.values()
            )
            total_parallel_sessions = sum(
                m.get("parallel_work_sessions", 0) for m in all_metrics.values()
            )
            avg_efficiency = sum(
                m.get("collaboration_efficiency", 0) for m in all_metrics.values()
            ) / len(all_metrics)
            ...
            for correlation_id, metrics in all_metrics.items():
                with st.expander(f"Workflow {correlation_id[-8:]}"):
                    ...
                    with col1:
                        st.write(
                            f"**Active Agents:** {len(metrics.get('active_agents', []))}"
                        )
                        st.write(
                            f"**Completed Agents:** {len(metrics.get('completed_agents', []))}"
                        )
                        st.write(
                            f"**Artifacts Created:** {metrics.get('total_artifacts', 0)}"
                        )

                    with col2:
                        st.write(
                            f"**Handoffs Completed:** {metrics.get('handoffs_completed', 0)}"
                        )
                        st.write(
                            f"**Parallel Sessions:** {metrics.get('parallel_work_sessions', 0)}"
                        )
                        st.write(
                            f"**Efficiency Score:** {metrics.get('collaboration_efficiency', 0):.3f}"
                        )
...
def render_collaboration_timeline(progress_tracker: LiveProgressTracker):
    ...
    if artifacts:
        ...
        timeline_data = (
            df.groupby(["time_bucket", "artifact_type"])
            .size()
...
def render_performance_trends():
    ...
    time_points = pd.date_range(
        start=datetime.now() - timedelta(hours=1), end=datetime.now(), freq="5T"
    )

    response_times = np.random.normal(
        2.5, 0.5, len(time_points)
    )  # Average 2.5s response
...
def render_demo_activity_feed():
    ...
    has_real_data = (
        hasattr(st.session_state, "agent_outputs")
        and st.session_state.agent_outputs
        and any(
            output.get("is_real", False) for output in st.session_state.agent_outputs
        )
    )
    ...
    if has_real_data:
        ...
        st.caption(
            f"Current time: {current_time} (All timestamps above are from real API execution)"
        )

    else:
        st.info("📊 No Real Execution Data - Start a real AI project to see live data")
        st.markdown(
            "Click 'Start REAL AI Project' on the Agent Orchestration tab to begin real execution."
        )
