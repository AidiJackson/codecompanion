...
class LiveStatusPanel:
    ...
    def get_recent_artifacts(
        self, task_id: Optional[str] = None, hours_back: int = 24
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            artifacts = self.db_manager.get_recent_artifacts(
                since_timestamp=since_time, task_id=task_id
            )
    ...
    def render_status_panel(self):
        ...
        with col1:
            ...
            if bus_status["health"] == "healthy":
                ...
                st.write(
                    f"Redis: {'Connected' if bus_status['redis_connected'] else 'Disconnected'}"
                )
    ...
    def render_live_timeline(
        self, task_id: Optional[str] = None, auto_refresh: bool = True
    ):
        ...
        for i, artifact in enumerate(artifacts):
            with st.container():
                ...
                with col4:
                    ...
                    if st.button("View Details", key=f"details_{artifact['id']}"):
                        # Show artifact details in expander
                        with st.expander(
                            f"Artifact Details - {artifact['id']}", expanded=True
                        ):
                            st.json(
                                {
                                    "id": artifact["id"],
...
def render_live_monitoring_dashboard(
    db_manager: DatabaseManager, task_id: Optional[str] = None
):
