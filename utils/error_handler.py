...
class APIRateLimiter:
    ...
    def record_failure(self, service_name: str):
        ...
        logger.warning(
            f"API failure recorded for {service_name}, retry count: {self.retry_counts[service_name]}"
        )
...
def safe_api_call(func: Callable, *args, timeout: int = 30, **kwargs) -> Dict[str, Any]:
    ...
    try:
        ...
        if "rate limit" in str(e).lower() or "quota" in str(e).lower():
            ...
            error_type = "auth"
            error_msg = (
                "Authentication failed. Please check your API keys in the sidebar."
            )
...
def memory_cleanup():
    ...
    try:
        ...
        if len(chat_history) > 50:
            ...
            logger.info(
                f"Cleaned up chat history, kept last 50 of {len(chat_history)} messages"
            )
        ...
        if total_size > 1000000:  # 1MB limit
            ...
            if len(project_files) > 10:
                ...
                logger.info(
                    f"Cleaned up project files, kept 10 of {len(project_files)} files"
                )
